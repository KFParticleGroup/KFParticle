...
class ALICETrackParamsConverter {
 ...
 private:
  ...
  template<typename T>
  Double_t GetSign(T x)
  { return (x > 0) ? 1 : -1; }

  void GlobalToLocal(Double_t xyz[3], Double_t pxpypz[3], Double_t cv[21], Double_t fP[5], Double_t fC[15],
                     Short_t sign);
  void GetCovarianceXYZPxPyPz(Double_t xyz[3], Double_t pxpypz[3], Double_t cv[21], Double_t fP[5], Double_t fC[15],
                              Short_t charge);
...
ALICETrackParamsConverter::ALICETrackParamsConverter(double xyz_[3], double pxpypz_[3], double cv_[21], short sign_)
{
  ...
  for (int i = 0; i < 21; i++) { globCV[i] = cv_[i]; }
...
ALICETrackParamsConverter::ALICETrackParamsConverter(double params_[5], double locCV_[15], double x_, double alpha_,
                                                     short sign_)
{
  for (int i = 0; i < 5; i++) { localParams[i] = params_[i]; }
  for (int i = 0; i < 15; i++) { locCV[i] = locCV_[i]; }
...
void ALICETrackParamsConverter::GlobalToLocal(Double_t xyz[3], Double_t pxpypz[3], Double_t cv[21], Double_t fP[5],
...
void ALICETrackParamsConverter::GetCovarianceXYZPxPyPz(Double_t xyz[3], Double_t pxpypz[3], Double_t cv[21],
                                                       Double_t fP[5], Double_t fC[15], Short_t charge)
{
  //---------------------------------------------------------------------
...
{
  std::cout << "parameters:\t" << xyz[0] << "\t" << xyz[1] << "\t" << xyz[2] << "\t" << pxpypz[0] << "\t" << pxpypz[1]
            << "\t" << pxpypz[2] << std::endl;
  ...
  std::cout << "covmat:\t"
            ...
            << "\n"
            << globCV[15] << "\t" << globCV[16] << "\t" << globCV[17] << "\t" << globCV[18] << "\t" << globCV[19]
            << "\t" << globCV[20] << "\n";
  cout << endl;
}
...
{
  std::cout << "parameters:\t" << localParams[0] << "\t" << localParams[1] << "\t" << localParams[2] << "\t"
            << localParams[3] << "\t" << localParams[4] << std::endl;
  std::cout << "sign:\t" << sign << std::endl;
  std::cout << "covmat:\t"
            << "\n"
            << locCV[0] << "\n"
