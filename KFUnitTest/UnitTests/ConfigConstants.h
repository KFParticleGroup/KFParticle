...
// MCParticleStruct& part){ .... } to produce covariance matrix for particle
// Leave only one of the files uncommented
#include "MakeCovMat/MakeCovMatALICE.h"
// #include ...

// header file with function std::vector<float> MakeVertexCovMatrix(const
// MCParticleStruct& part){ .... } to produce covariance matrix for particles'
// vertexes Leave only one of the files uncommented If there is no uncommented
// headers, topological constraint for mother will be set according to MC vertex
// .
#include "MakeCovMat/MakeVertexCovMatALICE.h"
// #include ...

// Settings of KFParticle version
//#define TESTSUITE  // uncomment if you are using Testsuite branch of
// KFParticle #define ALIPHYSICS   // uncomment if you are using KFParticle
// inside AliPhysics
...
const std::array<std::array<int, MAX_NUM_OF_DAUGHTERS>, NUM_OF_DECAYS>
    DAUGHTER_PDG_IDS = {{{LAMBDA_PDG, PIMINUS_PDG}, {PROTON_PDG, PIMINUS_PDG}}};
...
const bool SET_MASS_CONSTRAINT_DAUGHTERS =
    true; // set up nonlinear mass constraint

const bool SET_TOPOLOGICAL_CONSTRAINT_MOTHER =
    true; // must know covmat of primary vertex
...
const TString MCInputFile =
    "../MCData/Xi_cascade/Xi_to_Lambda0_to_Piminus_P_output.root"; //"../MCData/D0_to_Kminus_Piplus/D0_to_Kminus_Piplus_output.root";
...
const std::array<TString, NUM_OF_DECAYS> nameOfMCInitialOutputTree = {
    {"MCInitial_Xi", "MCInitial_Lambda"}};
const std::array<TString, NUM_OF_DECAYS> nameOfMCFinalOutputTree = {
    {"MCFinal_Xi", "MCFinal_Lambda"}};
const std::array<TString, NUM_OF_DECAYS> nameOfKFProductionVertexTree = {
    {"KFBeforeReco_Xi", "KFBeforeReco_Lambda"}};
const std::array<TString, NUM_OF_DECAYS> nameOfKFDecayVertexTree = {
    {"KFAfterReco_Xi", "KFAfterReco_Lambda"}};
