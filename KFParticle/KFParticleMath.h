...
namespace KFPMath {
static inline __attribute__((always_inline)) float32_v
multiplySign(const int32_v &sign, const float32_v &value) {
  const __m128 s = _mm_and_ps(_mm_castsi128_ps(_mm_set1_epi32(0x80000000)),
                              _mm_castsi128_ps(sign.simd()));
  return float32_v(_mm_xor_ps(value.simd(), s));
}

static inline __attribute__((always_inline)) float32_v
sinSeries(const float32_v x) {
...
sincos(float32_v x, float32_v &sinX, float32_v &cosX) {
...
static inline __attribute__((always_inline)) float32_v
ATan2(const float32_v &y, const float32_v &x) {
  ...
  const mask32_v &gt_tan_pi_8 =
      (a > float32_v(0.4142135623730950f)) && (!gt_tan_3pi_8);
  ...
  b +=
      (((8.05374449538e-2f * a2 - 1.38776856032E-1f) * a2 + 1.99777106478E-1f) *
           a2 -
       3.33329491539E-1f) *
          a2 * a +
      a;
