...
KFParticleFinder::KFParticleFinder()
  ...
  , fPrimCandidates()
  , fPrimCandidatesTopo()
  , fPrimCandidatesTopoMass()
  , fEmcClusters(0)
  , fMixedEventAnalysis(0)
  , fDecayReconstructionList()
{
...
void KFParticleFinder::Init(int nPV)
{
  ...
  for (int iCandidates = 0; iCandidates < fNSecCandidatesSets; iCandidates++) { fSecCandidates[iCandidates].clear(); }
                                     ...
                                     std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx, int nPV)
{
  ...
  //* Finds particles (K0s and Lambda) from a given set of tracks
  {
    ...
    for (int iV = 0; iV < 4; iV++) {
      for (int iTr = 0; iTr < vRTracks[iV].Size(); iTr++) {
        ...
        if (vRTracks[iV + 4].Size() > 0) { vRTracks[iV + 4].SetId(Particles.size(), iTr); }
        tmp.AddDaughterId(kfTrack.Id());
#ifdef NonhomogeneousField
        for (int iF = 0; iF < 10; iF++) { tmp.SetFieldCoeff(vRTracks[iV].FieldCoefficient(iF)[iTr], iF); }
#endif
  ...
  Find2DaughterDecay(vRTracks, ChiToPrimVtx, Particles, PrimVtx, fCuts2D, fSecCuts, fPrimCandidates, fSecCandidates);
  ...
  if (!fMixedEventAnalysis) {
    ...
    // K+ -> pi+ pi+ pi-
    FindTrackV0Decay(fPipi, 100310, vRTracks[0], 1, vRTracks[0].FirstPion(), vRTracks[0].LastPion(), Particles, PrimVtx,
                     -1, 0, &fPrimCandidates[11]);

    // K- -> pi+ pi- pi-
    FindTrackV0Decay(fPipi, 100310, vRTracks[1], -1, vRTracks[1].FirstPion(), vRTracks[1].LastPion(), Particles,
                     PrimVtx, -1, 0, &fPrimCandidates[12]);

    MatchKaons(vRTracks, PrimVtx, Particles);

    //Hypernuclei
    //H3L -> d p pi-, H4L -> d d pi-, H5L -> t d pi-
    FindTrackV0Decay(fDPi, 3003, vRTracks[0], 1, vRTracks[0].FirstProton(), vRTracks[0].LastBe7(), Particles, PrimVtx,
                     -1, 0);
    //H3L_bar -> d- p- pi+, H4L_bar -> d- d- pi+, H5L_bar -> t- d- pi+
    FindTrackV0Decay(fDPiBar, -3003, vRTracks[1], -1, vRTracks[1].FirstProton(), vRTracks[1].LastBe7(), Particles,
                     PrimVtx, -1, 0);
    //H4L -> t p pi-, H6L -> t t pi-
    FindTrackV0Decay(fTPi, 3103, vRTracks[0], 1, vRTracks[0].FirstProton(), vRTracks[0].LastBe7(), Particles, PrimVtx,
                     -1, 0);
    //H4L_bar -> t- p- pi+, H6L_bar -> t- t- pi+
    FindTrackV0Decay(fTPiBar, -3103, vRTracks[1], -1, vRTracks[1].FirstProton(), vRTracks[1].LastBe7(), Particles,
                     PrimVtx, -1, 0);
    //He4L -> He3 p pi-
    FindTrackV0Decay(fHe3Pi, 3004, vRTracks[0], 1, vRTracks[0].FirstProton(), vRTracks[0].LastBe7(), Particles, PrimVtx,
                     -1, 0);
    ...
    FindTrackV0Decay(fHe3PiBar, -3004, vRTracks[1], -1, vRTracks[1].FirstProton(), vRTracks[1].LastBe7(), Particles,
                     PrimVtx, -1, 0);
    //He5L -> He4 p pi-
    FindTrackV0Decay(fHe4Pi, 3005, vRTracks[0], 1, vRTracks[0].FirstProton(), vRTracks[0].LastBe7(), Particles, PrimVtx,
                     -1, 0);
    ...
    FindTrackV0Decay(fHe4PiBar, -3005, vRTracks[1], -1, vRTracks[1].FirstProton(), vRTracks[1].LastBe7(), Particles,
                     PrimVtx, -1, 0);
    //LLn -> H3L pi-
    ...
    FindTrackV0Decay(h3L, 3004, vRTracks[1], -1, vRTracks[1].FirstPion(), vRTracks[1].LastPion(), Particles, PrimVtx,
                     -1, 0);
    ...
    FindTrackV0Decay(h4L, 3005, vRTracks[1], -1, vRTracks[1].FirstPion(), vRTracks[1].LastPion(), Particles, PrimVtx,
                     -1, 0);
    ...
    //H4LL -> H3L p pi-
    FindTrackV0Decay(fLLn, 3203, vRTracks[0], 1, vRTracks[0].FirstProton(), vRTracks[0].LastProton(), Particles,
                     PrimVtx, -1, 0);
    //He6LL -> He5L p pi-
    FindTrackV0Decay(fH5LL, 3010, vRTracks[0], 1, vRTracks[0].FirstProton(), vRTracks[0].LastProton(), Particles,
                     PrimVtx, -1, 0);
    //H2L -> p p pi-
    FindTrackV0Decay(fPpi, 8122, vRTracks[0], 1, vRTracks[0].FirstProton(), vRTracks[0].LastProton(), Particles,
                     PrimVtx, -1, 0);
    //H2L_bar -> p- p- pi+
    FindTrackV0Decay(fPpiBar, -8122, vRTracks[1], -1, vRTracks[1].FirstProton(), vRTracks[1].LastProton(), Particles,
                     PrimVtx, -1, 0);
    //He3L -> p p p pi-
    FindTrackV0Decay(fPPpi, 3028, vRTracks[0], 1, vRTracks[0].FirstProton(), vRTracks[0].LastProton(), Particles,
                     PrimVtx, -1, 0);
    //He3L_bar -> p- p- p- pi+
    FindTrackV0Decay(fPPpiBar, -3028, vRTracks[1], -1, vRTracks[1].FirstProton(), vRTracks[1].LastProton(), Particles,
                     PrimVtx, -1, 0);
    //Li5L -> He3 p p pi-
    FindTrackV0Decay(fHe3PPi, 3006, vRTracks[0], 1, vRTracks[0].FirstProton(), vRTracks[0].LastProton(), Particles,
                     PrimVtx, -1, 0);
    //Li6L -> He4 p p pi-
    FindTrackV0Decay(fHe4PPi, 3007, vRTracks[0], 1, vRTracks[0].FirstProton(), vRTracks[0].LastProton(), Particles,
                     PrimVtx, -1, 0);
    ...
    for (int iPV = 0; iPV < fNPV; iPV++) {
      FindTrackV0Decay(fPrimCandidates[2][iPV], -3122, vRTracks[3], -1, vRTracks[3].FirstProton(),
                       vRTracks[3].LastHe4(), Particles, PrimVtx, iPV, 0);
    }
    // Charm
    //LambdaC -> pi+ K- p, Ds+ -> pi+ K- K+, D+ -> pi+ K- pi+
    FindTrackV0Decay(fD0, 421, vRTracks[0], 1, vRTracks[0].FirstPion(), vRTracks[0].LastProton(), Particles, PrimVtx,
                     -1, &(ChiToPrimVtx[0]));
    //LambdaC_bar -> pi- K+ p-, Ds- -> pi- K+ K-, D- -> pi- K+ pi-
    FindTrackV0Decay(fD0bar, -421, vRTracks[1], -1, vRTracks[1].FirstPion(), vRTracks[1].LastProton(), Particles,
                     PrimVtx, -1, &(ChiToPrimVtx[1]));
    //D0->pi+ K- pi+ pi-
    FindTrackV0Decay(fDPlus, 411, vRTracks[1], -1, vRTracks[1].FirstPion(), vRTracks[1].LastPion(), Particles, PrimVtx,
                     -1, &(ChiToPrimVtx[1]));
    //D0_bar->pi- K+ pi- pi+
    FindTrackV0Decay(fDMinus, -411, vRTracks[0], 1, vRTracks[0].FirstPion(), vRTracks[0].LastPion(), Particles, PrimVtx,
                     -1, &(ChiToPrimVtx[0]));
    //B+ -> D0_bar pi+, B+ -> D0_bar K+
    FindTrackV0Decay(fD0bar, -421, vRTracks[0], 1, vRTracks[0].FirstPion(), vRTracks[0].LastKaon(), Particles, PrimVtx,
                     -1, &(ChiToPrimVtx[0]));
    //B- -> D0 pi-, B- -> D0 K-
    FindTrackV0Decay(fD0, 421, vRTracks[1], -1, vRTracks[1].FirstPion(), vRTracks[1].LastKaon(), Particles, PrimVtx, -1,
                     &(ChiToPrimVtx[1]));
    //B0 -> D- pi+, B0 -> D- K+
    FindTrackV0Decay(fDMinus, -419, vRTracks[0], 1, vRTracks[0].FirstPion(), vRTracks[0].LastKaon(), Particles, PrimVtx,
                     -1, &(ChiToPrimVtx[0]));
    //B0_bar -> D+ pi-, B0_bar -> D+ K-
    FindTrackV0Decay(fDPlus, 419, vRTracks[1], -1, vRTracks[1].FirstPion(), vRTracks[1].LastKaon(), Particles, PrimVtx,
                     -1, &(ChiToPrimVtx[1]));
    //D0 -> pi+ K-
    SelectParticles(Particles, fD0, PrimVtx, fCutsCharm[2], fCutsCharm[1], KFParticleDatabase::Instance()->GetD0Mass(),
                    KFParticleDatabase::Instance()->GetD0MassSigma(), fSecCuts[0]);
    //D0_bar -> pi+ K-
    SelectParticles(Particles, fD0bar, PrimVtx, fCutsCharm[2], fCutsCharm[1],
                    KFParticleDatabase::Instance()->GetD0Mass(), KFParticleDatabase::Instance()->GetD0MassSigma(),
                    fSecCuts[0]);
    //D*+->D0 pi+
    for (int iPV = 0; iPV < fNPV; iPV++) {
      FindTrackV0Decay(fD0, 421, vRTracks[2], 1, vRTracks[2].FirstPion(), vRTracks[2].LastPion(), Particles, PrimVtx,
                       iPV, 0);
    }
    //D*- -> D0_bar pi-
    for (int iPV = 0; iPV < fNPV; iPV++) {
      FindTrackV0Decay(fD0, -421, vRTracks[3], -1, vRTracks[3].FirstPion(), vRTracks[3].LastPion(), Particles, PrimVtx,
                       iPV, 0);
    }
    //D0 -> pi+ K- pi+ pi-
    SelectParticles(Particles, fD04, PrimVtx, fCutsCharm[2], fCutsCharm[1], KFParticleDatabase::Instance()->GetD0Mass(),
                    KFParticleDatabase::Instance()->GetD0MassSigma(), fSecCuts[0]);
    //D0_bar -> pi- K+ pi- pi+
    ...
    //D*+->D0 pi+
    for (int iPV = 0; iPV < fNPV; iPV++) {
      FindTrackV0Decay(fD04, 429, vRTracks[2], 1, vRTracks[2].FirstPion(), vRTracks[2].LastPion(), Particles, PrimVtx,
                       iPV, 0);
    }
    //D0*- -> D0_bar pi-
    for (int iPV = 0; iPV < fNPV; iPV++) {
      FindTrackV0Decay(fD04bar, -429, vRTracks[3], -1, vRTracks[3].FirstPion(), vRTracks[3].LastPion(), Particles,
                       PrimVtx, iPV, 0);
    }
    //D+
    SelectParticles(Particles, fDPlus, PrimVtx, fCutsCharm[2], fCutsCharm[1],
                    KFParticleDatabase::Instance()->GetDPlusMass(), KFParticleDatabase::Instance()->GetDPlusMassSigma(),
                    fSecCuts[0]);
    //D-
    ...
    //D*0->D+ pi-
    for (int iPV = 0; iPV < fNPV; iPV++) {
      FindTrackV0Decay(fDPlus, 411, vRTracks[3], -1, vRTracks[3].FirstPion(), vRTracks[3].LastPion(), Particles,
                       PrimVtx, iPV, 0);
    }
    //D*0_bar->D- pi+
    for (int iPV = 0; iPV < fNPV; iPV++) {
      FindTrackV0Decay(fDMinus, -411, vRTracks[2], 1, vRTracks[2].FirstPion(), vRTracks[2].LastPion(), Particles,
                       PrimVtx, iPV, 0);
    }
    ...
    FindTrackV0Decay(fD0pipi, 420, vRTracks[0], 1, vRTracks[0].FirstPion(), vRTracks[0].LastProton(), Particles,
                     PrimVtx, -1, &(ChiToPrimVtx[0]));
    //LambdaC_bar -> p_bar pi+ pi-, Ds- -> K- pi+ pi-, D- -> pi+ pi- pi-
    FindTrackV0Decay(fD0pipi, 420, vRTracks[1], -1, vRTracks[1].FirstPion(), vRTracks[1].LastProton(), Particles,
                     PrimVtx, -1, &(ChiToPrimVtx[1]));
    ...
    //Lambdac+ -> p K0s
    for (int iPV = 0; iPV < fNPV; iPV++) {
      FindTrackV0Decay(fPrimCandidates[0][iPV], 310, vRTracks[0], 1, vRTracks[0].FirstProton(),
                       vRTracks[0].LastProton(), Particles, PrimVtx, -1, &(ChiToPrimVtx[0]));
    }
    //Lambdac_bar- -> p_bar K0s
    for (int iPV = 0; iPV < fNPV; iPV++) {
      FindTrackV0Decay(fPrimCandidates[0][iPV], 310, vRTracks[1], -1, vRTracks[1].FirstProton(),
                       vRTracks[1].LastProton(), Particles, PrimVtx, -1, &(ChiToPrimVtx[1]));
    }

    //H0 -> Lambda Lambda
    CombinePartPart(fSecCandidates[1], fSecCandidates[1], Particles, PrimVtx, fCutsPartPart[0], -1, 3000, 1, 1);
    //H0 -> Lambda p pi-
    FindTrackV0Decay(fLPi, 3002, vRTracks[0], 1, vRTracks[0].FirstProton(), vRTracks[0].LastProton(), Particles,
                     PrimVtx, -1);
    //Sigma0 -> Lambda Gamma
    for (int iPV = 0; iPV < fNPV; iPV++) {
      CombinePartPart(fPrimCandidates[3][iPV], fPrimCandidates[1][iPV], Particles, PrimVtx, fCutsPartPart[1], iPV,
                      3212);
    }
    //Sigma0_bar -> Lambda_bar Gamma
    for (int iPV = 0; iPV < fNPV; iPV++) {
      CombinePartPart(fPrimCandidates[3][iPV], fPrimCandidates[2][iPV], Particles, PrimVtx, fCutsPartPart[1], iPV,
                      -3212);
    }
    //pi0 -> gamma gamma
    const float& mPi0      = KFParticleDatabase::Instance()->GetPi0Mass();
    const float& mPi0Sigma = KFParticleDatabase::Instance()->GetPi0MassSigma();
    CombinePartPart(fSecCandidates[3], fSecCandidates[3], Particles, PrimVtx, fCutsPartPart[1], -1, 111, 1, 0,
                    &fPrimCandidates[4], &fSecCandidates[4], mPi0, mPi0Sigma);
    for (int iPV = 0; iPV < fNPV; iPV++) {
      CombinePartPart(fPrimCandidates[3][iPV], fPrimCandidates[3][iPV], Particles, PrimVtx, fCutsPartPart[1], iPV, 111,
                      1, 0, &fPrimCandidates[4], &fSecCandidates[4], mPi0, mPi0Sigma);
      CombinePartPart(fSecCandidates[3], fPrimCandidates[3][iPV], Particles, PrimVtx, fCutsPartPart[1], -1, 111, 0, 0,
                      &fPrimCandidates[4], &fSecCandidates[4], mPi0, mPi0Sigma);
    }
    for (int iPV = 0; iPV < fNPV; iPV++) { ExtrapolateToPV(fPrimCandidates[4][iPV], PrimVtx[iPV]); }
    //eta -> pi0 pi0 pi0
    //TODO implement this
    //Sigma+ -> p pi0
    ...
    //K*0 -> K0 pi0
    for (int iPV = 0; iPV < fNPV; iPV++) {
      CombinePartPart(fPrimCandidates[4][iPV], fPrimCandidates[0][iPV], Particles, PrimVtx, fCutsPartPart[1], iPV,
                      100313, 0, 1);
    }
    //Sigma*0 -> Lambda pi0
    for (int iPV = 0; iPV < fNPV; iPV++) {
      CombinePartPart(fPrimCandidates[4][iPV], fPrimCandidates[1][iPV], Particles, PrimVtx, fCutsPartPart[1], iPV, 3214,
                      0, 1);
    }
    //Sigma*0_bar -> Lambda_bar pi0
    for (int iPV = 0; iPV < fNPV; iPV++) {
      CombinePartPart(fPrimCandidates[4][iPV], fPrimCandidates[2][iPV], Particles, PrimVtx, fCutsPartPart[1], iPV,
                      -3214, 0, 1);
    }
    //Xi*- -> Xi- pi0
    for (int iPV = 0; iPV < fNPV; iPV++) {
      CombinePartPart(fPrimCandidates[4][iPV], fPrimCandidates[5][iPV], Particles, PrimVtx, fCutsPartPart[1], iPV, 3314,
                      0, 1);
    }
    //Xi*+ -> Xi+ pi0
    for (int iPV = 0; iPV < fNPV; iPV++) {
      CombinePartPart(fPrimCandidates[4][iPV], fPrimCandidates[6][iPV], Particles, PrimVtx, fCutsPartPart[1], iPV,
                      -3314, 0, 1);
    }
    //JPsi -> Lambda Lambda_bar
    for (int iPV = 0; iPV < fNPV; iPV++) {
      CombinePartPart(fPrimCandidates[1][iPV], fPrimCandidates[2][iPV], Particles, PrimVtx, fCutsPartPart[1], iPV,
                      300443, 0, 1);
    }
    //JPsi -> Xi- Xi+
    for (int iPV = 0; iPV < fNPV; iPV++) {
      CombinePartPart(fPrimCandidates[5][iPV], fPrimCandidates[6][iPV], Particles, PrimVtx, fCutsPartPart[1], iPV,
                      400443, 0, 1);
    }
    //Psi -> Omega- Omega+
    for (int iPV = 0; iPV < fNPV; iPV++) {
      CombinePartPart(fPrimCandidates[7][iPV], fPrimCandidates[8][iPV], Particles, PrimVtx, fCutsPartPart[1], iPV,
                      500443, 0, 1);
    }
...
void KFParticleFinder::ExtrapolateToPV(vector<KFParticle>& vParticles, KFParticleSIMD& PrimVtx)
{
  ...
  for (int iv = 0; iv < SimdLen; iv++) { parts[iv] = &tmpPart[iv]; }
  ...
  for (unsigned int iL = 0; iL < vParticles.size(); iL += SimdLen) {
    ...
    for (unsigned int iv = 0; iv < nEntries; iv++) { tmpPart[iv] = vParticles[iL + iv]; }

    KFParticleSIMD tmp(parts, nEntries);

    tmp.TransportToPoint(PrimVtx.Parameters());

    for (unsigned int iv = 0; iv < nEntries; iv++) { tmp.GetKFParticle(vParticles[iL + iv], iv); }
  }
}

inline void KFParticleFinder::ConstructV0(KFPTrackVector* vTracks, int iTrTypePos, int iTrTypeNeg,
                                          int32_v& idPosDaughters, int32_v& idNegDaughters, int32_v& daughterPosPDG,
                                          int32_v& daughterNegPDG, KFParticleSIMD& mother, KFParticle& mother_temp,
                                          const unsigned short NTracks, kfvector_floatv& l, kfvector_floatv& dl,
                                          vector<KFParticle>& Particles,
                                          std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx,
                                          const float* cuts, const int32_v& pvIndex, const float* secCuts,
                                          const float32_v& massMotherPDG, const float32_v& massMotherPDGSigma,
                                          KFParticleSIMD& motherPrimSecCand, int& nPrimSecCand,
                                          vector<vector<KFParticle>>* vMotherPrim, vector<KFParticle>* vMotherSec)
{
  ...
  saveParticle &=
    (((isK0 || isLambda || isHyperNuclei) && lMin > float32_v(fLCut)) || !(isK0 || isLambda || isHyperNuclei));
  ...
  for (int iv = 0; iv < NTracks; iv++) {
    ...
    if (mother.PDG()[iv] == 3003) { fDPi.push_back(mother_temp); }
    if (mother.PDG()[iv] == -3003) { fDPiBar.push_back(mother_temp); }
    if (mother.PDG()[iv] == 3103) { fTPi.push_back(mother_temp); }
    if (mother.PDG()[iv] == -3103) { fTPiBar.push_back(mother_temp); }
    if (mother.PDG()[iv] == 3004) { fHe3Pi.push_back(mother_temp); }
    if (mother.PDG()[iv] == -3004) { fHe3PiBar.push_back(mother_temp); }
    if (mother.PDG()[iv] == 3005) { fHe4Pi.push_back(mother_temp); }
    if (mother.PDG()[iv] == -3005) { fHe4PiBar.push_back(mother_temp); }
    ...
    if (mother.PDG()[iv] == 200113) {
      ...
      if ((negPt2 > fCutJPsiPt * fCutJPsiPt) && (posPt2 > fCutJPsiPt * fCutJPsiPt) && (abs(daughterPosPDG[iv]) == 13)
          && (abs(daughterNegPDG[iv]) == 13)) {
...
inline void KFParticleFinder::SaveV0PrimSecCand(KFParticleSIMD& mother, int& NParticles, KFParticle& mother_temp,
                                                std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx,
                                                const float* secCuts, vector<vector<KFParticle>>* vMotherPrim,
                                                vector<KFParticle>* vMotherSec)
{
  ...
  for (int iv = 0; iv < NParticles; iv++) {
    if (isPrim[iv] || isSec[iv]) {
      ...
      if (isSec[iv]) { vMotherSec[arrayIndex[iv]].push_back(mother_temp); }
    }
  }
}
...
void KFParticleFinder::Find2DaughterDecay(KFPTrackVector* vTracks, kfvector_float* ChiToPrimVtx,
                                          vector<KFParticle>& Particles,
                                          std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx,
                                          const float* cuts, const float* secCuts,
                                          vector<vector<KFParticle>>* vMotherPrim, vector<KFParticle>* vMotherSec)
{
  ...
  for (int iTrTypeNeg = 0; iTrTypeNeg < 2; iTrTypeNeg++) {
    ...
    for (int iTrTypePos = 0; iTrTypePos < 2; iTrTypePos++) {
      ...
      if ((iTrTypeNeg == 0) && (iTrTypePos == 0)) {
        ...
        // e-
        startTCPos[0] = 0;
        endTCPos[0]   = posTracks.LastElectron();  //nPositiveTracks;
        startTCNeg[0] = 0;
        endTCNeg[0]   = negTracks.LastElectron();  //negTracksSize[0];
        //mu-
        startTCPos[1] = 0;
        endTCPos[1]   = 0;
        startTCNeg[1] = 0;
          ...
          for (int iTrP = startTCPos[iTC]; iTrP < endTCPos[iTC]; iTrP += SimdLen) {
            ...
            for (int iRot = 0; iRot < SimdLen; iRot++) {
              ...
              for (int iPDGPos = 0; iPDGPos < nPDGPos; iPDGPos++) {
                ...
                if (!fMixedEventAnalysis) {
                  if (iTC == 0) {
                    motherPDG = select((abs(trackPdgPos[iPDGPos]) == 11) && (abs(trackPdgNeg) == 11) && isSecondary, 22,
                                       motherPDG);  //gamma -> e+ e-
                    ...
                    motherPDG = select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 13 || abs(trackPdgPos[iPDGPos]) == 19)
                                         && ((abs(trackPdgNeg) == 13) || (abs(trackPdgNeg) == 19)),
                                       200113, motherPDG);  //rho -> mu+ mu-
                  }
                  else if (iTC == 2) {
                    motherPDG = select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 211) && (abs(trackPdgNeg) == 211),
                                       310, motherPDG);  //K0 -> pi+ pi-
                    motherPDG = select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 2212) && (abs(trackPdgNeg) == 211),
                                       3122, motherPDG);  //Lambda -> p+ pi-
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 1000010020) && (abs(trackPdgNeg) == 211),
                             3003, motherPDG);  //LambdaN -> d+ pi-
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 1000010030) && (abs(trackPdgNeg) == 211),
                             3103, motherPDG);  //LambdaNN -> t+ pi-
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 1000020030) && (abs(trackPdgNeg) == 211),
                             3004, motherPDG);  //H3Lambda -> He3+ pi-
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 1000020040) && (abs(trackPdgNeg) == 211),
                             3005, motherPDG);  //H4Lambda -> He4+ pi-
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 1000020060) && (abs(trackPdgNeg) == 211),
                             3016, motherPDG);  //H6Lambda -> He6+ pi-
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 1000030060) && (abs(trackPdgNeg) == 211),
                             3019, motherPDG);  //He6Lambda -> Li6+ pi-
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 1000030070) && (abs(trackPdgNeg) == 211),
                             3022, motherPDG);  //He7Lambda -> Li7+ pi-
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 1000040070) && (abs(trackPdgNeg) == 211),
                             3025, motherPDG);  //Li7Lambda -> Be7+ pi-
                    motherPDG = select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 321) && (abs(trackPdgNeg) == 211),
                                       -421, motherPDG);  //D0_bar -> pi- K+
                    motherPDG = select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 321) && (abs(trackPdgNeg) == 211),
                                       313, motherPDG);  //K*0 -> K+ pi-
                    motherPDG = select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 211) && (abs(trackPdgNeg) == 211),
                                       113, motherPDG);  //rho -> pi+ pi-
                    motherPDG = select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 2212) && (abs(trackPdgNeg) == 211),
                                       2114, motherPDG);  //Delta0 -> p pi-

                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000010020) && (abs(trackPdgNeg) == 211),
                             100001, motherPDG);  //d   pi-
                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000010030) && (abs(trackPdgNeg) == 211),
                             100003, motherPDG);  //t   pi-
                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000020030) && (abs(trackPdgNeg) == 211),
                             100005, motherPDG);  //He3 pi-
                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000020040) && (abs(trackPdgNeg) == 211),
                             100007, motherPDG);  //He4 pi-
                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000020060) && (abs(trackPdgNeg) == 211),
                             100009, motherPDG);  //He6 pi-
                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000030060) && (abs(trackPdgNeg) == 211),
                             100011, motherPDG);  //Li6 pi-
                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000030070) && (abs(trackPdgNeg) == 211),
                             100013, motherPDG);  //Li7 pi-
                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000040070) && (abs(trackPdgNeg) == 211),
                             100015, motherPDG);  //Be7 pi-
                  }
                  else if (iTC == 3) {
                    motherPDG = select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 211) && (abs(trackPdgNeg) == 321),
                                       421, motherPDG);  //D0 -> pi+ K-
                    motherPDG = select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 321) && (abs(trackPdgNeg) == 321),
                                       426, motherPDG);  //D0 -> K+ K-
                    motherPDG = select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 211) && (abs(trackPdgNeg) == 321),
                                       -313, motherPDG);  //K*0_bar -> K- pi+
                    motherPDG = select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 2212) && (abs(trackPdgNeg) == 321),
                                       3124, motherPDG);  //Lambda* -> p K-
                    motherPDG = select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 321) && (abs(trackPdgNeg) == 321),
                                       333, motherPDG);  //phi -> K+ K-

                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000010020) && (abs(trackPdgNeg) == 321),
                             110001, motherPDG);  //d   K-
                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000010030) && (abs(trackPdgNeg) == 321),
                             110003, motherPDG);  //t   K-
                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000020030) && (abs(trackPdgNeg) == 321),
                             110005, motherPDG);  //He3 K-
                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000020040) && (abs(trackPdgNeg) == 321),
                             110007, motherPDG);  //He4 K-
                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000020060) && (abs(trackPdgNeg) == 321),
                             110009, motherPDG);  //He6 K-
                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000030060) && (abs(trackPdgNeg) == 321),
                             110011, motherPDG);  //Li6 K-
                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000030070) && (abs(trackPdgNeg) == 321),
                             110013, motherPDG);  //Li7 K-
                    motherPDG =
                      select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 1000040070) && (abs(trackPdgNeg) == 321),
                             110015, motherPDG);  //Be7 K-
                  }
                  else if (iTC == 4) {
                    motherPDG = select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 211) && (abs(trackPdgNeg) == 2212),
                                       -3122, motherPDG);  //Lambda_bar -> p- pi+
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 211) && (abs(trackPdgNeg) == 1000010020),
                             -3003, motherPDG);  //LambdaN_bar -> d- pi+
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 211) && (abs(trackPdgNeg) == 1000010030),
                             -3103, motherPDG);  //LambdaNN_bar -> t- pi+
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 211) && (abs(trackPdgNeg) == 1000020030),
                             -3004, motherPDG);  //H3Lambda_bar -> He3- pi+
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 211) && (abs(trackPdgNeg) == 1000020040),
                             -3005, motherPDG);  //H4Lambda_bar -> He4- pi+
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 211) && (abs(trackPdgNeg) == 1000020060),
                             -3016, motherPDG);  //H6Lambda_bar -> He6- pi+
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 211) && (abs(trackPdgNeg) == 1000030060),
                             -3019, motherPDG);  //He6Lambda_bar -> Li6- pi+
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 211) && (abs(trackPdgNeg) == 1000030070),
                             -3022, motherPDG);  //Li6Lambda_bar -> Li7- pi+
                    motherPDG =
                      select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 211) && (abs(trackPdgNeg) == 1000040070),
                             -3025, motherPDG);  //Li7Lambda_bar -> Be7- pi+
                    motherPDG = select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 321) && (abs(trackPdgNeg) == 2212),
                                       -3124, motherPDG);  //Lambda*_bar -> p- K+
                    motherPDG = select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 2212) && (abs(trackPdgNeg) == 2212),
                                       200443, motherPDG);  //JPsi -> p- p
                    motherPDG = select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 211) && (abs(trackPdgNeg) == 2212),
                                       -2114, motherPDG);  //Delta0_bar -> p- pi+
                  }
                }
                else {
                  if (iTC == 0) {
                    motherPDG = select((abs(trackPdgPos[iPDGPos]) == 11) && (abs(trackPdgNeg) == 11), 22,
                                       motherPDG);  //gamma -> e+ e-
                  }
                  else if (iTC == 1) {
                    motherPDG = select(isPrimary && (abs(trackPdgPos[iPDGPos]) == 13 || abs(trackPdgPos[iPDGPos]) == 19)
                ...
                if (!((iTrTypePos == 1) && (iTrTypeNeg == 1))) {
                  ...
                  float32_v p1p2 = posParameters[3] * negParameters[3] + posParameters[4] * negParameters[4]
                                   + posParameters[5] * negParameters[5];
                  float32_v p12  = posParameters[3] * posParameters[3] + posParameters[4] * posParameters[4]
                                   + posParameters[5] * posParameters[5];
                  float32_v p22  = negParameters[3] * negParameters[3] + negParameters[4] * negParameters[4]
                                   + negParameters[5] * negParameters[5];
                ...
                if (!((abs(motherPDG) == 421 || abs(motherPDG) == 426).isEmpty())) {
                  active[iPDGPos] &= ((abs(motherPDG) == 421 || abs(motherPDG) == 426)
                                      && (ptNeg2 >= fCutCharmPt * fCutCharmPt) && (ptPos2 >= fCutCharmPt * fCutCharmPt)
                ...
                for (int iV = 0; iV < SimdLen; iV++) {
                  ...
                  if (int(nBufEntry) == SimdLen) {
                    ...
                    mother.SetPDG(V0PDG);
                    ConstructV0(vTracks, trTypeIndexPos[iTrTypePos], trTypeIndexNeg[iTrTypeNeg], idPosDaughters,
                                idNegDaughters, daughterPosPDG, daughterNegPDG, mother, mother_temp, nBufEntry, l, dl,
                                Particles, PrimVtx, cuts, pvIndexMother, secCuts, massMotherPDG, massMotherPDGSigma,
                                motherPrimSecCand, nPrimSecCand, vMotherPrim, vMotherSec);
                    nBufEntry = 0;
                  }
...
void KFParticleFinder::Find2DaughterDecayOneSign(KFPTrackVector& vTracks, std::vector<KFParticle>& Particles,
                                                 std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx)
{
  KFParticle mother_temp;
  kfvector_floatv l(fNPV), dl(fNPV);
...
void KFParticleFinder::ConstructResonance(const std::vector<KFParticle>& particles1,
                                          const std::vector<KFParticle>& particles2, const int motherPdg,
...
void KFParticleFinder::ConstructResonances2D(KFPTrackVector* vTracks, std::vector<KFParticle>& Particles,
                                             std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx)
{
...
void KFParticleFinder::ConstructPrimaryBG(KFPTrackVector* vTracks, vector<KFParticle>& Particles,
                                          std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx,
                                          const float* cuts, const float* secCuts,
                                          vector<vector<KFParticle>>* vMotherPrim, vector<KFParticle>* vMotherSec)
{
      ...
      for (int iTr2 = iTr1 + 1; iTr2 < nPositiveTracks; iTr2++) {
...

        {
          ...
          if (int(nBufEntry) == SimdLen) {
            ...
            KFParticleDatabase::Instance()->GetMotherMass(V0PDG, massMotherPDG, massMotherPDGSigma);
            mother.SetPDG(V0PDG);
            ConstructV0(vTracks, iSet, iSet, idPosDaughters, idNegDaughters, daughterPosPDG, daughterNegPDG, mother,
                        mother_temp, nBufEntry, l, dl, Particles, PrimVtx, cuts, pvIndexMother, secCuts, massMotherPDG,
...
void KFParticleFinder::ConstructTrackV0Cand(
  KFPTrackVector& vTracks, int32_v& idTracks, int32_v& trackPDG, KFParticle* vV0[], KFParticleSIMD& mother,
  std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& motherTopo, KFParticle& mother_temp,
  const unsigned short nElements, kfvector_floatv& l, kfvector_floatv& dl, std::vector<KFParticle>& Particles,
  std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx, const float32_v* cuts, const int32_v& pvIndex,
  const float32_v& massMotherPDG, const float32_v& massMotherPDGSigma,
  std::vector<std::vector<KFParticle>>* vMotherPrim, std::vector<KFParticle>* vMotherSec)
{
  ...
  mask32_v isSameParticle = ((abs(mother.PDG()) == int32_v(4122)) || (abs(mother.PDG()) == int32_v(114122))
                             || (abs(mother.PDG()) == int32_v(204122)) || (abs(mother.PDG()) == int32_v(504122))
                             || (abs(mother.PDG()) == int32_v(404122)) || (abs(mother.PDG()) == int32_v(425))
                             || (abs(mother.PDG()) == int32_v(427)) || (abs(mother.PDG()) == int32_v(200411))
                             || (abs(mother.PDG()) == int32_v(300411)) || (abs(mother.PDG()) == int32_v(300431))
                             || (abs(mother.PDG()) == int32_v(400431)) || (abs(mother.PDG()) == int32_v(411))
                             || (abs(mother.PDG()) == int32_v(431)) || (abs(mother.PDG()) == int32_v(429))
                             || (abs(mother.PDG()) == int32_v(1003334)) || (abs(mother.PDG()) == int32_v(3001))
                             || (abs(mother.PDG()) == int32_v(3006)) || (abs(mother.PDG()) == int32_v(3007))
                             || (abs(mother.PDG()) == int32_v(3009)) || (abs(mother.PDG()) == int32_v(100321))
  ...
  for (int iP = 0; iP < fNPV; iP++) {
    ...
    isPrimaryPart |= isPrimaryPartLocal;
    for (int iV = 0; iV < SimdLen; iV++) {
      if (isPrimaryPartLocal[iV]) { iPrimVert[iV].push_back(iP); }
    }
  }
  ...
  for (unsigned int iv = 0; iv < nElements; iv++) {
    ...
    if ((abs(mother.PDG()[iv]) == 411) || (abs(mother.PDG()[iv]) == 429) || (abs(mother.PDG()[iv]) == 431)
        || (abs(mother.PDG()[iv]) == 4122) || (abs(mother.PDG()[iv]) == 114122) || (abs(mother.PDG()[iv]) == 204122)
        || (abs(mother.PDG()[iv]) == 314122) || (abs(mother.PDG()[iv]) == 404122) || (abs(mother.PDG()[iv]) == 504122)
        || (abs(mother.PDG()[iv]) == 425) || (abs(mother.PDG()[iv]) == 427) || (abs(mother.PDG()[iv]) == 200411)
        || (abs(mother.PDG()[iv]) == 300411) || (abs(mother.PDG()[iv]) == 300431) || (abs(mother.PDG()[iv]) == 400431)
        || (abs(mother.PDG()[iv]) == 1003334) || (abs(mother.PDG()[iv]) == 3001)) {
      mother_temp.CleanDaughtersId();
      for (int iD = 0; iD < vV0[iv]->NDaughters(); iD++) { mother_temp.AddDaughterId(vV0[iv]->DaughterIds()[iD]); }
    ...
    if (mother.PDG()[iv] == 3203) { fLLn.push_back(mother_temp); }
    if (mother.PDG()[iv] == 3010) { fH5LL.push_back(mother_temp); }
    if (mother.PDG()[iv] == 3028) { fPPpi.push_back(mother_temp); }
    if (mother.PDG()[iv] == -3028) { fPPpiBar.push_back(mother_temp); }

    if (mother.PDG()[iv] == 3006) { fHe3PPi.push_back(mother_temp); }
    if (mother.PDG()[iv] == 3007) { fHe4PPi.push_back(mother_temp); }
    ...
    if (!(isPrimaryPart[iv]) /*|| (mother.PDG()[iv] == 3006) || (mother.PDG()[iv] == 3007) */) {
      if (vMotherSec) {
        ...
        if (abs(mother.PDG()[iv]) == 3324) { vMotherSec->push_back(mother_temp); }
        else {
    ...
    if (abs(mother.GetPDG()[iv]) == 521 || abs(mother.GetPDG()[iv]) == 529 || abs(mother.GetPDG()[iv]) == 511
        || abs(mother.GetPDG()[iv]) == 519) {
    ...
    if (abs(mother.GetPDG()[iv]) == 3334)  //Omega-
    {
      ...
      if (mother.GetPDG()[iv] == 3334) { motherVector = &fPrimCandidates[8]; }
...
void KFParticleFinder::FindTrackV0Decay(vector<KFParticle>& vV0, const int V0PDG, KFPTrackVector& vTracks, const int q,
                                        const int firstTrack, const int lastTrack, vector<KFParticle>& Particles,
                                        std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx,
                                        int v0PVIndex, kfvector_float* ChiToPrimVtx,
                                        vector<vector<KFParticle>>* vMotherPrim, vector<KFParticle>* vMotherSec)
{
    ...
    for (int iTr = firstTrack; iTr < lastTrack; iTr += SimdLen) {
      ...
      for (int iPDGPos = 0; iPDGPos < nPDGPos; iPDGPos++) {
        ...
        if (V0PDG == 3122) {
          ...
          const int32_v& id = reinterpret_cast<const int32_v&>(vTracks.Id()[iTr]);
          motherPDG =
            select(isSecondary && (trackPdgPos[iPDGPos] == 1000010020) && (id > iPosDaughter), 3014, motherPDG);
          ...
          const int32_v& id = reinterpret_cast<const int32_v&>(vTracks.Id()[iTr]);
          motherPDG =
            select(isSecondary && (trackPdgPos[iPDGPos] == -1000010020) && (id > iNegDaughter), -3014, motherPDG);
          motherPDG = select(isSecondary && (trackPdgPos[iPDGPos] == -1000010030), -3015, motherPDG);
        }
        else if (V0PDG == 3103) {
          ...
          const int32_v& id = reinterpret_cast<const int32_v&>(vTracks.Id()[iTr]);
          motherPDG =
            select(isSecondary && (trackPdgPos[iPDGPos] == 1000010030) && (id > iPosDaughter), 3017, motherPDG);
        }
        else if (V0PDG == -3103) {
          motherPDG         = select(isSecondary && (trackPdgPos[iPDGPos] == -2212), -3013, motherPDG);
          const int32_v& id = reinterpret_cast<const int32_v&>(vTracks.Id()[iTr]);
          motherPDG =
            select(isSecondary && (trackPdgPos[iPDGPos] == -1000010030) && (id > iNegDaughter), -3017, motherPDG);
        }
        else if (V0PDG == 3004) {
          ...
          const int32_v& id = reinterpret_cast<const int32_v&>(vTracks.Id()[iTr]);
          motherPDG =
            select(isSecondary && (trackPdgPos[iPDGPos] == 1000020030) && (id > iPosDaughter), 3024, motherPDG);
          ...
          const int32_v& id = reinterpret_cast<const int32_v&>(vTracks.Id()[iTr]);
          motherPDG =
            select(isSecondary && (trackPdgPos[iPDGPos] == -1000020030) && (id > iNegDaughter), -3024, motherPDG);
          motherPDG = select(isSecondary && (trackPdgPos[iPDGPos] == -1000020040), -3026, motherPDG);
        }
        else if (V0PDG == 3005) {
          ...
          const int32_v& id = reinterpret_cast<const int32_v&>(vTracks.Id()[iTr]);
          motherPDG =
            select(isSecondary && (trackPdgPos[iPDGPos] == 1000020040) && (id > iPosDaughter), 3027, motherPDG);
          ...
          const int32_v& id = reinterpret_cast<const int32_v&>(vTracks.Id()[iTr]);
          motherPDG =
            select(isSecondary && (trackPdgPos[iPDGPos] == -1000020040) && (id > iNegDaughter), -3027, motherPDG);
        }
        else if (V0PDG == 3006) {
          const int32_v& id = reinterpret_cast<const int32_v&>(vTracks.Id()[iTr]);
          motherPDG =
            select(isSecondary && (trackPdgPos[iPDGPos] == 2212) && (id > vV0[iV0].DaughterIds()[2]), 3038, motherPDG);
          motherPDG = select(isSecondary && (trackPdgPos[iPDGPos] == -211) && (id > iNegDaughter), 3008, motherPDG);
        }
        else if (V0PDG == 3007) {
          const int32_v& id = reinterpret_cast<const int32_v&>(vTracks.Id()[iTr]);
          motherPDG =
            select(isSecondary && (trackPdgPos[iPDGPos] == 2212) && (id > vV0[iV0].DaughterIds()[2]), 3039, motherPDG);
          ...
          const int32_v& id = reinterpret_cast<const int32_v&>(vTracks.Id()[iTr]);
          motherPDG =
            select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 2212) && (id > iPosDaughter), 3028, motherPDG);
        }
        else if (V0PDG == -8122) {
          const int32_v& id = reinterpret_cast<const int32_v&>(vTracks.Id()[iTr]);
          motherPDG =
            select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 2212) && (id > iNegDaughter), -3028, motherPDG);
        }
        else if (V0PDG == 3028) {
          const int32_v& id = reinterpret_cast<const int32_v&>(vTracks.Id()[iTr]);
          motherPDG = select(isSecondary && (abs(trackPdgPos[iPDGPos]) == 2212) && (id > vV0[iV0].DaughterIds()[2]),
                             3029, motherPDG);
        }
        else if (V0PDG == -3028) {
          const int32_v& id = reinterpret_cast<const int32_v&>(vTracks.Id()[iTr]);
        ...
        if (ChiToPrimVtx) {
          active[iPDGPos] &=
            (!((abs(motherPDG) == 3334 || abs(motherPDG) == 3312))
             || ((abs(motherPDG) == 3334 || abs(motherPDG) == 3312)
                 && (reinterpret_cast<const float32_v&>((*ChiToPrimVtx)[iTr]) > float32_v(fCuts2D[0]))));
        }
        ...
        if (isCharm) {
          ...
          active[iPDGPos] &= (trackPt >= fCutCharmPt * fCutCharmPt)
                             && (reinterpret_cast<const float32_v&>((*ChiToPrimVtx)[iTr]) > fCutCharmChiPrim)
                             && (nPixelHits >= int32_v(3));
        }
        {
          mask32_v isCharmParticle = (abs(motherPDG) == 104122) || (abs(motherPDG) == 204122)
                                     || (abs(motherPDG) == 304122) || (abs(motherPDG) == 404122)
                                     || (abs(motherPDG) == 425) || (abs(motherPDG) == 426) || (abs(motherPDG) == 427)
        ...
        for (int iV = 0; iV < NTracks; iV++) {
          ...
          for (int iCut = 0; iCut < 3; iCut++) { cutsArray[iCut][nBufEntry] = fCutsTrackV0[motherType][iCut]; }
          ...
          if (int(nBufEntry) == SimdLen) {
            ...
            ConstructTrackV0Cand(vTracks, idTrack, trackPDGMother, v0Pointer, mother, motherTopo, mother_temp,
                                 nBufEntry, l, dl, Particles, PrimVtx, cuts, pvIndexMother, massMotherPDG,
  ...
  if (nBufEntry > 0) {
    ...
    ConstructTrackV0Cand(vTracks, idTrack, trackPDGMother, v0Pointer, mother, motherTopo, mother_temp, nBufEntry, l, dl,
                         Particles, PrimVtx, cuts, pvIndexMother, massMotherPDG, massMotherPDGSigma, vMotherPrim,
                         vMotherSec);
    nBufEntry = 0;
  }
}
...
void KFParticleFinder::SelectParticles(vector<KFParticle>& Particles, vector<KFParticle>& vCandidates,
                                       std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx,
                                       const float& cutChi2Topo, const float& cutLdL, const float& mass,
                                       const float& massErr, const float& massCut)
{
  ...
  for (int iC = 0; iC < nCand; iC += SimdLen) {
    ...
    for (int iv = 0; iv < nEntries; iv++) { cand[iv] = &vCandidates[iC + iv]; }
    ...
    for (int iv = 0; iv < nEntries; iv++) {
      ...
      for (int iP = 0; iP < fNPV; iP++) {
        ...
        if (float(candTopo[iP].GetChi2()[iv]) / float(candTopo[iP].GetNDF()[iv]) <= cutChi2Topo) { isPrimary = 1; }
      }
...
void KFParticleFinder::CombinePartPart(vector<KFParticle>& particles1, vector<KFParticle>& particles2,
                                       vector<KFParticle>& Particles,
                                       std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx,
                                       const float* cuts, int iPV, const int MotherPDG, bool isSameInputPart,
                                       bool saveOnlyPrimary, vector<vector<KFParticle>>* vMotherPrim,
                                       vector<KFParticle>* vMotherSec, float massMotherPDG, float massMotherPDGSigma)
{
  ...
  if (!(fDecayReconstructionList.empty())
      && (fDecayReconstructionList.find(MotherPDG) == fDecayReconstructionList.end())) {
    return;
  }
    ...
    for (int iP2 = startIndex; iP2 < nPart2; iP2 += SimdLen) {
      ...
      for (int iv = 0; iv < nElements; iv++) {
        ...
        if (vMotherPrim || vMotherSec) {
          ...
          if (vMotherSec && (!(isPrimaryPart[iv]))) { vMotherSec->push_back(mother_temp); }
          if (vMotherPrim) {
            for (unsigned int iP = 0; iP < iPrimVert[iv].size(); iP++) {
              (*vMotherPrim)[iPrimVert[iv][iP]].push_back(mother_temp);
            }
                                  ...
                                  std::vector<KFParticle>& Particles)
{
  ...
  for (int iKaonSet = 0; iKaonSet < nKaonSets; iKaonSet++) {
    if (!(fDecayReconstructionList.empty())
        && (fDecayReconstructionList.find(newPdg[iKaonSet]) == fDecayReconstructionList.end())) {
      continue;
    }
                                            ...
                                            std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx)
{
  ...
  for (int iTrTypeDaughter = 0; iTrTypeDaughter < 2; iTrTypeDaughter++) {
    ...
    //He3
    startTCMother[4]   = 0;
    endTCMother[4]     = MotherTracksSize;
    startTCDaughter[4] = DaughterTracks.FirstHe3();
    endTCDaughter[4]   = DaughterTracks.LastHe3();
    ...
    //He4
    startTCMother[5]   = 0;
    endTCMother[5]     = MotherTracksSize;
    startTCDaughter[5] = DaughterTracks.FirstHe4();
    endTCDaughter[5]   = DaughterTracks.LastHe4();
        ...
        for (unsigned short iTrM = startTCMother[iTC]; iTrM < endTCMother[iTC]; iTrM += SimdLen) {
          ...
          for (int iRot = 0; iRot < SimdLen; iRot++) {
            if (iRot > 0) {
              ...
              activeDaughter = /*( (DaughterPDG != -1) || ( (DaughterPVIndex < 0) && (DaughterPDG == -1) ) ) &&*/ (
                DaughterInd < DaughterTracksSize);
            }

            int32_v trackPdgMother;

            if (iTC == 0) { activeDaughter &= abs(DaughterPDG) == 13; }
            if (iTC == 1) { activeDaughter &= abs(DaughterPDG) == 211; }
            if (iTC == 2) { activeDaughter &= abs(DaughterPDG) == 321; }
            ...
            for (int iHypothesis = 0; iHypothesis < nMotherHypothesis[iTC]; iHypothesis++) {
              ...
              if (iTrTypeDaughter == 0) { motherKFPDG = -outMotherPDG[iTC][iHypothesis]; }
              if (!(fDecayReconstructionList.empty())
                  && (fDecayReconstructionList.find(motherKFPDG) == fDecayReconstructionList.end())) {
                continue;
              }
              ...
              float32_v yMother  = MotherTrack.Y();
              float32_v yCD      = ChargedDaughter.Y();
              float32_v distance = sqrt((zMother - zCD) * (zMother - zCD) + (xMother - xCD) * (xMother - xCD)
                                        + (yMother - yCD) * (yMother - yCD));
              float32_v lMotherTrak, dlMotherTrak;
              MotherTrack.GetDistanceToVertexLine(PrimVtx[0], lMotherTrak, dlMotherTrak);
              float32_v lChargedTrak, dChargedTrak;
              ChargedDaughter.GetDistanceToVertexLine(PrimVtx[0], lChargedTrak, dChargedTrak);
              ...
              neutralDaughter.NDF()  = -1;
              neutralDaughter.Chi2() = 0.f;
              neutralDaughter.ReconstructMissingMass(ChargedDaughter, MotherFiltered, cDaughterFiltered,
                                                     neutralDaughterMassHypothesis[iTC][iHypothesis]);

              float32_v neutralMass, neutralMassError;
              neutralDaughter.GetMass(neutralMass, neutralMassError);
              if (iTC > 0) { active &= (neutralMass > 0.f); }
              ...
              for (int iV = 0; iV < NTracks; iV++) {
                ...
                int neutralId = Particles.size();
                mother_temp.SetId(neutralId);
                if (iTrTypeDaughter == 0) { mother_temp.SetPDG(-outNeutralDaughterPDG[iTC][iHypothesis]); }
                else {
                  mother_temp.SetPDG(outNeutralDaughterPDG[iTC][iHypothesis]);
                }
                Particles.push_back(mother_temp);
                ...
                mother_temp.AddDaughterId(ChargedDaughter.Id()[iV]);
                mother_temp.AddDaughterId(neutralId);
                if (iTrTypeDaughter == 0) { mother_temp.SetPDG(-outMotherPDG[iTC][iHypothesis]); }
                else {
                  mother_temp.SetPDG(outMotherPDG[iTC][iHypothesis]);
                }
                Particles.push_back(mother_temp);
...
void KFParticleFinder::AddCandidate(const KFParticle& candidate, int iPV)
{
  ...
  if (iSet > -1) {
    if (iPV >= 0 && iPV < fNPV) {
      if (candidate.NDF() == 2) { fPrimCandidates[iSet][iPV].push_back(candidate); }
      if (candidate.NDF() == 3) { fPrimCandidatesTopo[iSet][iPV].push_back(candidate); }
      if (candidate.NDF() == 4) { fPrimCandidatesTopoMass[iSet][iPV].push_back(candidate); }
    }
    else if (iPV < 0) {
