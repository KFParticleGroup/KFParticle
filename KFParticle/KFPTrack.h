...
/** @class KFPTrack
 ** @brief A scalar class for storage of the track in the cartesian
 *parametrisation.
 ...
 ** track, chi2 of the track fit, the corresponding number of degrees of
 *freedom,
...
#endif
{
 ...
 public:
  ...
  bool GetCovarianceXYZPxPyPz(float cv[21]) const
  {
     ...
     ** \param[out] cv[21] - the output array, where the covariance matrix is
     *copied
     **/
  ...
  /** Copies position of the track to the output array of floats. \param[out]
   * position - the output array with the position of the track **/
  ...
  /** Copies position of the track to the output array of doubles. \param[out]
   * position - the output array with the position of the track **/
  ...
  float GetPt() const
  { return sqrt(fP[3] * fP[3] + fP[4] * fP[4]); }  ///< Returns Pt - transverse momentum of the track.
  float GetP() const
  { return sqrt(fP[3] * fP[3] + fP[4] * fP[4] + fP[5] * fP[5]); }  ///< Returns P - momentum of the track.

  void GetCovarianceMatrix(float* covmatrix)
  {
    /** Copies the covariance matrix of the track to the array of floats.
     ** \param[out] covmatrix[21] - the output array, where the covariance
     *matrix is copied
     **/
    for (int i = 0; i < 21; i++) { covmatrix[i] = fC[i]; }
  }
  float GetParameter(int i) const
  { return fP[i]; }  ///< Returns parameter "i" of the track. \param[in] i - index of the
  ///< parameter to be returned
  float GetCovariance(int i) const
  { return fC[i]; }  ///< Returns element of the covariance matrix "i" of the track. \param[in] i
  ...
  { return fC; }  ///< Returns a pointer to the array of the covariance matrix elements stored
  ///< in a lower triangular form.

  void SetParameters(const float* position)
  {
    /** Sets parameters { X, Y, Z, Px, Py, Pz } of the track from the input
     *array of floats.
  ...
  void SetCovarianceMatrix(const float* C)
  {
     ...
     ** \param[in] C[21] - array with the input elements of the covariance
     *matrix stored in the lower triangular form
     **/
  ...
  /** Sets an element of the covariance matrix with index "i". \param[in] c -
   * value to be set \param[in] i - index of the element */
  void SetCovariance(const int i, const float c) { fC[i] = c; }

  void RotateXY(float alpha);  // rotate on alpha in XY plane. Should be useful
                               // for CS change
  ...
  { return fieldRegion; }  ///< Returns array of the coefficients for field approximation.
  /** Sets a field coefficient with index "i". \param[in] c - value to be set
 ...
 private:
  ...
  float fC[21];  ///< Covariance matrix of the track parameters. Stored in the
