...
/** @class KFPVertex
 ** @brief A scalar class for storage of the vertex in the cartesian
 *parametrisation.
...
public:
  ...
  /** Copies position of the vertex to the output array of floats. \param[out]
   * position - the output array with the position of the vertex **/
  void GetXYZ(float *position) const {
    position[0] = fP[0];
    position[1] = fP[1];
    position[2] = fP[2];
  }
  /** Copies position of the vertex to the output array of doubles. \param[out]
   * position - the output array with the position of the vertex **/
  ...
  void GetCovarianceMatrix(float *covmatrix) const {
     ...
     ** \param[out] covmatrix[6] - the output array, where the covariance matrix
     *is copied
     **/
    for (int i = 0; i < 6; i++)
      covmatrix[i] = fC[i];
  ...
  float GetParameter(int i) const {
    return fP[i];
  } ///< Returns parameter "i" of the vertex. \param[in] i - index of the
    ///< parameter to be returned
  float GetCovariance(int i) const {
    return fC[i];
  } ///< Returns element of the covariance matrix "i" of the vertex. \param[in]
  ...
  void SetCovarianceMatrix(float *C) {
     ...
     ** \param[in] C[6] - array with the input elements of the covariance matrix
     *stored in the lower triangular form
     **/
    for (int i = 0; i < 6; i++)
      fC[i] = C[i];
  }

  void SetCovarianceMatrix(float C00, float C10, float C11, float C20,
                           float C21, float C22) {
    /** Sets the covariance matrix from the input array of floats.
     ** \param[in] C00 - Cxx
