...
public:
  ...
  void SetTracks(const KFPEmcCluster &track, const kfvector_int &trackIndex,
                 const int nIndexes);
  ...
  const kfvector_int &Id() const {
    return fId;
  } ///< Returns constant reference to the vector with unique id of the
    ///< clusters.
  ...
  void SetCovariance(float value, int iC, int iTr) {
    fC[iC][iTr] = value;
  } ///< Sets the "value" of the element of covariance matrix "iC" of the
...
private:
  ...
  kfvector_float
      fC[10]; ///< Covariance matrix of the parameters of the cluster.
