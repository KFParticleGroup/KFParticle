 ...
 ** @brief Allocator which is needed to allocate memory in std::vector aligned
 *by the size of SIMD vectors.
 ** @author  M.Zyzak, I.Kisel
 ** @date 05.02.2019
 ** @version 1.0
 **/

template <class T> class KFPSimdAllocator {
public:
  // type definitions
   ...
   ** @version 1.0
   **/
  template <class U> struct rebind { typedef KFPSimdAllocator<U> other; };
  ...
  template <class U> KFPSimdAllocator(const KFPSimdAllocator<U> &) throw() {}
  ...
  void *operator new(size_t size) {
    return _mm_malloc(size, sizeof(KFP::SIMD::SimdSize));
  } ///< new operator for allocation of the SIMD-alligned dynamic memory
    ///< allocation
  void *operator new[](size_t size) {
    return _mm_malloc(size, sizeof(KFP::SIMD::SimdSize));
