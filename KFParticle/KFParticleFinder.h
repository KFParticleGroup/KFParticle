...
class KFParticleFinder {
 public:
  ...
  { fNThreads = n; }  ///< Sets the number of threads to by run in parallel. Currently not used.

  void FindParticles(KFPTrackVector* vRTracks, kfvector_float* ChiToPrimVtx, std::vector<KFParticle>& Particles,
  ...
  void ConstructResonance(const std::vector<KFParticle>& particles1, const std::vector<KFParticle>& particles2,
                          const int motherPdg, std::vector<KFParticle>& output);

  void ConstructResonances2D(KFPTrackVector* vTracks, std::vector<KFParticle>& Particles,
                             std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx);

  void SaveV0PrimSecCand(KFParticleSIMD& mother, int& NParticles, KFParticle& mother_temp,
  ...
  void Find2DaughterDecayOneSign(KFPTrackVector& vTracks, std::vector<KFParticle>& Particles,
                                 std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx);

  void ConstructPrimaryBG(KFPTrackVector* vTracks, std::vector<KFParticle>& Particles,
                          std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx, const float* cuts,
                          const float* secCuts, std::vector<std::vector<KFParticle>>* vMotherPrim,
                          std::vector<KFParticle>* vMotherSec);
  ...
  void FindTrackV0Decay(std::vector<KFParticle>& vV0, const int V0PDG, KFPTrackVector& vTracks, const int q,
                        const int firstTrack, const int lastTrack, std::vector<KFParticle>& Particles,
                        std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx, int v0PVIndex = -1,
                        kfvector_float* ChiToPrimVtx = 0, std::vector<std::vector<KFParticle>>* vMotherPrim = 0,
                        std::vector<KFParticle>* vMotherSec = 0);

  void FindLL(const int motherPDG, const int correctTrackPDG, const std::vector<KFParticle>& vParticles,
              const KFPTrackVector& vTracks, const int firstTrack, const int lastTrack, const KFParticleSIMD& PrimVtx,
              std::vector<KFParticle>& Particles);

  void SelectParticles(std::vector<KFParticle>& Particles, std::vector<KFParticle>& vCandidates,
                       std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx, const float& cutChi2Topo,
                       const float& cutLdL, const float& mass, const float& massErr, const float& massCut);

  void CombinePartPart(std::vector<KFParticle>& particles1, std::vector<KFParticle>& particles2,
                       std::vector<KFParticle>& Particles,
                       std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx, const float* cuts,
                       int iPV, const int MotherPDG, bool isSameInputPart = 0, bool saveOnlyPrimary = 1,
                       std::vector<std::vector<KFParticle>>* vMotherPrim = 0, std::vector<KFParticle>* vMotherSec = 0,
                       float massMotherPDG = 0.f, float massMotherPDGSigma = 0.f);

  void MatchKaons(KFPTrackVector* vTracks, std::vector<KFParticleSIMD, KFPSimdAllocator<KFParticleSIMD>>& PrimVtx,
                  std::vector<KFParticle>& Particles);

  // Set Emc clusters containing gammas
  void SetEmcClusters(KFPEmcCluster* clusters)
  { fEmcClusters = clusters; }  ///< Set a pointer to the gamma-clusters from the electromagnetic
  ///< calorimeter.
   ...
   *\f$\overline{\Lambda}\rightarrow \overline{p}\pi^+\f$
   ** with topological and mass constraints set on. **/
  const std::vector<std::vector<KFParticle>>& GetPrimaryTopoMassAntiLambda() const
  ...
  { fCutsTrackV0[0][0] = cut; }  ///< Sets \f$l/\Delta l\f$ cut for \f$\Xi\f$ and \f$\Omega\f$.
  void SetChi2TopoCutXiOmega(float cut)
  { fCutsTrackV0[0][1] = cut; }  ///< Sets \f$\chi^2_{topo}\f$ cut for \f$\Xi\f$ and \f$\Omega\f$.
  ...
  { fCutLVMPt = cut; }  ///< Sets the cut on transverse momentum of each daughter track of low mass
  ///< vector mesons.
  void SetPCutLMVM(float cut)
  { fCutLVMP = cut; }  ///< Sets the cut on momentum of each daughter track of low mass vector
  ///< mesons in dimuon channel.
  void SetPtCutJPsi(float cut) { fCutJPsiPt = cut; }  ///< Sets the cut on transverse momentum of each daughter track of
  ///< \f$J/\psi\f$.

  void SetPtCutCharm(float cut)
  { fCutCharmPt = cut; }  ///< Sets the cut on transverse momentum of each daughter track of open
  ///< charm particles.
  void SetChiPrimaryCutCharm(float cut)
  { fCutCharmChiPrim = cut; }  ///< Sets cut on \f$\chi^2_{prim}\f$ of each track for open charm particles.
  void SetLdLCutCharmManybodyDecays(float cut)
  { fCutsTrackV0[1][0] = cut; }  ///< Sets \f$l/\Delta l\f$ cut for open charm with >=3 daughters.
  void SetChi2TopoCutCharmManybodyDecays(float cut)
  { fCutsTrackV0[1][1] = cut; }  ///< Sets \f$\chi^2_{topo}\f$ cut for open charm with >=3 daughters.
  void SetChi2CutCharmManybodyDecays(float cut)
  { fCutsTrackV0[1][2] = cut; }  ///< Sets \f$\chi^2_{geo}\f$ cut for open charm with >=3 daughters.

  void SetLdLCutCharm2D(float cut)
  { fCutsCharm[1] = cut; }  ///< Sets \f$l/\Delta l\f$ cut for open charm with 2 daughters.
  void SetChi2TopoCutCharm2D(float cut)
  { fCutsCharm[2] = cut; }  ///< Sets \f$\chi^2_{topo}\f$ cut for open charm with 2 daughters.
  void SetChi2CutCharm2D(float cut)
  { fCutsCharm[0] = cut; }  ///< Sets \f$\chi^2_{geo}\f$ cut for open charm with 2 daughters.
  ...
  void CopyCuts(const KFParticleFinder* finder)
  {
    ...
    fLCut        = finder->fLCut;
    for (int iCut = 0; iCut < 3; iCut++) { fCuts2D[iCut] = finder->fCuts2D[iCut]; }
    for (int iCut = 0; iCut < 3; iCut++) { fSecCuts[iCut] = finder->fSecCuts[iCut]; }
    for (int iCut = 0; iCut < 3; iCut++) {
      for (int jCut = 0; jCut < 3; jCut++) { fCutsTrackV0[iCut][jCut] = finder->fCutsTrackV0[iCut][jCut]; }
    }
    for (int iCut = 0; iCut < 2; iCut++) {
      for (int jCut = 0; jCut < 3; jCut++) { fCutsPartPart[iCut][jCut] = finder->fCutsPartPart[iCut][jCut]; }
    }
    fCutCharmPt      = finder->fCutCharmPt;
    fCutCharmChiPrim = finder->fCutCharmChiPrim;
    for (int iCut = 0; iCut < 3; iCut++) { fCutsCharm[iCut] = finder->fCutsCharm[iCut]; }
  ...
  { fDecayReconstructionList = decays; }  ///< Set enitre reconstruction list
 ...
 private:
  ...
  float fDistanceCut;  ///< Cut on the distance between secondary tracks at the DCA
