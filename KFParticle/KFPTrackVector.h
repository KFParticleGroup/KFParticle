 ...
 ** @brief A class to store vectors of input tracks in the cartesian
 *parametrisation.
 ...
 ** random access to the data members. Tracks are sorted by
 *KFParticleTopoReconstructor::SortTracks():
 ** electrons, muons, pions, tracks without PID, kaons, protons, deuterons,
 *tritons, He3, He4, He6, Li6, Li7, Be7, Sigmas.
 **/
...
public:
  KFPTrackVector()
      : fId(), fPDG(), fQ(), fPVIndex(), fNPixelHits(), fNE(0), fNMu(0),
        fNPi(0), fNK(0), fNP(0), fND(0), fNT(0), fNHe3(0), fNHe4(0), fNHe6(0),
        fNLi6(0), fNLi7(0), fNBe7(0), fNSigma(0) {}
  ~KFPTrackVector() {}

  /**Returns size of the vectors. All data vectors have the same size. */
  int Size() const { return fP[0].size(); }
  int DataSize() const {
    /**Returns size of the memory in floats (4 bytes or 32 bits) allocated by
     * the current object. */
  ...
  void SetTracks(const KFPTrackVector &track, const kfvector_int &trackIndex,
                 const int nIndexes);
  ...
  const kfvector_float &Px() const {
    return fP[3];
  } ///< Returns constant reference to the vector with Px components of
    ///< momentum.
  const kfvector_float &Py() const {
    return fP[4];
  } ///< Returns constant reference to the vector with Py components of
    ///< momentum.
  const kfvector_float &Pz() const {
    return fP[5];
  } ///< Returns constant reference to the vector with Pz components of
    ///< momentum.

  const kfvector_float &Parameter(const int i) const {
    return fP[i];
  } ///< Returns constant reference to the track parameter vector with index
    ///< "i".
  const kfvector_float &Covariance(const int i) const {
    return fC[i];
  } ///< Returns constant reference to the vector of the covariance matrix
    ///< elements with index "i".
#ifdef NonhomogeneousField
  const kfvector_float &FieldCoefficient(const int i) const {
    return fField[i];
  } ///< Returns constant reference to the magnetic field coefficient with index
    ///< "i".
#endif

  const kfvector_int &Id() const {
    return fId;
  } ///< Returns constant reference to the vector with track Id
    ///< KFPTrackVector::fId.
  const kfvector_int &PDG() const {
    return fPDG;
  } ///< Returns constant reference to the vector with assigned PDG hypothesis
    ///< KFPTrackVector::fPDG.
  const kfvector_int &Q() const {
    return fQ;
  } ///< Returns constant reference to the vector with charge
    ///< KFPTrackVector::fQ.
  const kfvector_int &PVIndex() const {
    return fPVIndex;
  } ///< Returns constant reference to the vector with indices of corresponding
    ///< primary vertex KFPTrackVector::fPVIndex.
  const kfvector_int &NPixelHits() const {
    return fNPixelHits;
  } ///< Returns constant reference to the vector with the number of precise
    ///< measurements KFPTrackVector::fNPixelHits.

  float Pt(const int n) const {
    return sqrt(fP[3][n] * fP[3][n] + fP[4][n] * fP[4][n]);
  ...
  void SetCovariance(float value, int iC, int iTr) {
    fC[iC][iTr] = value;
  } ///< Sets the "value" of the element of covariance matrix "iC" of the track
    ///< with index "iTr".
  ...
  void SetFieldCoefficient(float value, int iP, int iTr) {
    fField[iP][iTr] = value;
  } ///< Sets the "value" of the field coefficient "iP" of the track with index
    ///< "iTr".
#endif
  ...
  void SetPVIndex(int value, int iTr) {
    fPVIndex[iTr] = value;
  } ///< Sets index of the corresponding primary vertex of the track with index
    ///< "iTr".
  void SetNPixelHits(int value, int iTr) {
    fNPixelHits[iTr] = value;
  ...
  void RecalculateLastIndex() {
    /** Recalculate the last index of each track specie. Should be called after
     * track sorting. */
    fNE = 0;
    fNMu = 0;
    fNPi = 0;
  ...
  int FirstMuon() {
    return int(fNE / SimdLen) * SimdLen;
  } ///< Returns index of the first element of the SIMD vector with the first
    ///< muon.
  ...
  int FirstPion() {
    return int(fNMu / SimdLen) * SimdLen;
  } ///< Returns index of the first element of the SIMD vector with the first
    ///< pion.
  ...
  int FirstKaon() {
    return int(fNPi / SimdLen) * SimdLen;
  } ///< Returns index of the first element of the SIMD vector with the first
    ///< kaon.
  ...
  int FirstProton() {
    return int(fNK / SimdLen) * SimdLen;
  } ///< Returns index of the first element of the SIMD vector with the first
    ///< proton.
  ...
  int FirstDeuteron() {
    return int(fNP / SimdLen) * SimdLen;
  } ///< Returns index of the first element of the SIMD vector with the first
    ///< deuteron.
  ...
  int FirstTritium() {
    return int(fND / SimdLen) * SimdLen;
  } ///< Returns index of the first element of the SIMD vector with the first
    ///< triton.
  ...
  int FirstHe3() {
    return int(fNT / SimdLen) * SimdLen;
  } ///< Returns index of the first element of the SIMD vector with the first
    ///< He3.
  ...
  int FirstHe4() {
    return int(fNHe3 / SimdLen) * SimdLen;
  } ///< Returns index of the first element of the SIMD vector with the first
    ///< He4.
  ...
  int FirstHe6() {
    return int(fNHe4 / SimdLen) * SimdLen;
  } ///< Returns index of the first element of the SIMD vector with the first
    ///< He6.
  const int &LastHe6() const {
    return fNHe6;
  }                                     ///< Returns index of the last He6.
  int NHe6s() { return fNHe6 - fNHe4; } ///< Returns number of He6 tracks.
  int FirstLi6() {
    return int(fNHe6 / SimdLen) * SimdLen;
  } ///< Returns index of the first element of the SIMD vector with the first
    ///< Li6.
  const int &LastLi6() const {
    return fNLi6;
  }                                     ///< Returns index of the last Li6.
  int NLi6s() { return fNLi6 - fNHe6; } ///< Returns number of Li6 tracks.
  int FirstLi7() {
    return int(fNLi6 / SimdLen) * SimdLen;
  } ///< Returns index of the first element of the SIMD vector with the first
    ///< Li7.
  const int &LastLi7() const {
    return fNLi7;
  }                                     ///< Returns index of the last Li7.
  int NLi7s() { return fNLi7 - fNLi6; } ///< Returns number of Li7 tracks.
  int FirstBe7() {
    return int(fNLi7 / SimdLen) * SimdLen;
  } ///< Returns index of the first element of the SIMD vector with the first
    ///< Be7.
  const int &LastBe7() const {
    return fNBe7;
  }                                     ///< Returns index of the last Be7.
  int NBe7s() { return fNBe7 - fNLi7; } ///< Returns number of Be7 tracks.
  int FirstSigma() {
    return int(fNBe7 / SimdLen) * SimdLen;
  } ///< Returns index of the first element of the SIMD vector with the first
    ///< Sigma.
  const int &LastSigma() const {
    return fNSigma;
  } ///< Returns index of the last Sigma.
  ...
  const KFPTrackVector &operator=(const KFPTrackVector &track) {
    /** Operator to copy one KFPTrackVector object to another. Makes one-to-one
     * copy.*/
  ...
  void SetDataToVector(int *data, int &offset) {
    /** Copies entire vector to the provided memory starting form the position
     *"offset".
     ** The function is used in KFPInputData::SetDataToVector().
     ** \param[out] data - pointer to the memory where the track vectors should
     *be copied; since all fields of
     ** KFPTrackVector are of the same size (int or float) pointer can be safely
     *casted to int*
     ** \param[in,out] offset - starting position in "data" where vectors should
     *be copied; after all vectors are
     ** copied the offset is shifted on the size of the written object so the
     *next KFPTrackVector object
  ...
  void ReadDataFromVector(int *data, int &offset) {
    /** Copies entire vector from the provided memory starting form the position
     *"offset".
     ** The function is used in KFPInputData::ReadDataFromVector().
     ** \param[in] data - pointer to the memory with the track vectors; since
     *all fields of
     ** KFPTrackVector are of the same size (int or float) pointer can be safely
     *casted to int*
     ** \param[in,out] offset - starting position of the memory to be copied;
     *after all vectors are
     ** copied the offset is shifted on the size of the read object so the next
...
private:
  kfvector_float
      fP[6]; ///< Vectors with parameters of the track : X, Y, Z, Px, Py, Pz.
  kfvector_float fC[21]; ///< Vectors with covariance matrix of the track
                         ///< parameters stroed in a lower triangular form.
  ...
  kfvector_int
      fPVIndex; ///< Vector with the index of the corresponding primary vertex.
                ///< If track is considered secondary "-1" is stored.
  kfvector_int
      fNPixelHits; ///< Vector with the number of hits from precise detectors
                   ///< (like MVD in CBM, HFT in STAR, ITS in ALICE, etc.)

  /** The coefficients of the field approximation of each field component along
   *the track trajectory using parabolas: \n
   ** cx0 = fField[0], cx1 = fField[1], cx2 = fField[2] - coefficients of the Bx
   *approximation; \n
   ** cy0 = fField[3], cy1 = fField[4], cy2 = fField[5] - coefficients of the By
   *approximation; \n
   ** cz0 = fField[6], cz1 = fField[7], cz2 = fField[8] - coefficients of the Bz
   *approximation; \n
...
inline void KFPTrackVector::SetParameter(const float32_v &value, int iP,
                                         int iTr) {
   ...
   ** \param[in] iTr - starting position in the parameter vector where the
...
inline void KFPTrackVector::SetCovariance(const float32_v &value, int iC,
                                          int iTr) {
...
inline void KFPTrackVector::GetTrack(KFPTrack &track, const int n) {
  /** Copies track with index "n" for the current object to the KFPTrack object
   *"track".
   ** \param[out] track - KFPTrack object, where track with index "n" is copied
   ** \param[in] n - index of the track to be copied
   **/
  track.SetParameters(fP[0][n], fP[1][n], fP[2][n], fP[3][n], fP[4][n],
                      fP[5][n]);
