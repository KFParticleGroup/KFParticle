...
#if defined(HomogeneousField) && defined(NonhomogeneousField)
#error                                                                         \
    "Both HomogeneousField and NonhomogeneousField are defined — choose only one!"
#endif

class KFParticle;

/** @class KFParticleSIMD
 ** @brief The main vectorised class of KF Particle pacakge, describes particle
 *objects.
 ** @author  M.Zyzak
 ** @date 05.02.2019
 ** @version 1.0
 ...
 ** to transport partices, get parameters together with their erros, get
 *distance
...
public:
  ...
  void SetField(const KFParticleFieldRegion &field) {
     ...
     ** \param[in] field - approximation of each component of the magnetic field
     *with the parabola alnog the particle trajectory
     **/
    fField = field;
  }
#endif
  ...
  KFParticleSIMD()
      : fQ(0), fNDF(-3), fChi2(0.f), SumDaughterMass(0.f), fMassHypo(-1.f),
        fId(-1), fPDG(0), fConstructMethod(0), fDaughterIds()
#ifdef NonhomogeneousField
        ,
        fField()
#endif
  ...
  //* Parameters, covariance matrix, charge and PID hypothesis should be
  // provided

  void Create(const float32_v Param[], const float32_v Cov[], int32_v Charge,
              float32_v mass /*int PID*/);
  ...
  KFParticleSIMD(KFPEmcCluster &track, int32_v &index,
                 const KFParticleSIMD &vertexGuess);
  KFParticleSIMD(KFPEmcCluster &track, int index,
                 const KFParticleSIMD &vertexGuess);
  void Create(KFPEmcCluster &track, int32_v &index,
              const KFParticleSIMD &vertexGuess);
  ...
  float32_v GetX() const {
    return fP[0];
  } ///< Returns the sum of masses of the daughters
  float32_v GetY() const {
    return fP[1];
  } ///< Returns the sum of masses of the daughters
  float32_v GetZ() const {
    return fP[2];
  } ///< Returns the sum of masses of the daughters
  float32_v GetPx() const {
    return fP[3];
  } ///< Returns the sum of masses of the daughters
  float32_v GetPy() const {
    return fP[4];
  } ///< Returns the sum of masses of the daughters
  float32_v GetPz() const {
    return fP[5];
  } ///< Returns the sum of masses of the daughters
  float32_v GetE() const {
    return fP[6];
  } ///< Returns the sum of masses of the daughters
  float32_v GetS() const {
    return fP[7];
  } ///< Returns the sum of masses of the daughters
  int32_v GetQ() const {
    return fQ;
  } ///< Returns the sum of masses of the daughters
  float32_v GetChi2() const {
    return fChi2;
  } ///< Returns the sum of masses of the daughters
  int32_v GetNDF() const {
    return fNDF;
  } ///< Returns the sum of masses of the daughters

  const float32_v &X() const {
    return fP[0];
  } ///< Retruns X coordinate of the particle, fP[0].
  const float32_v &Y() const {
    return fP[1];
  } ///< Retruns Y coordinate of the particle, fP[1].
  const float32_v &Z() const {
    return fP[2];
  } ///< Retruns Z coordinate of the particle, fP[2].
  const float32_v &Px() const {
    return fP[3];
  } ///< Retruns X component of the momentum, fP[3].
  const float32_v &Py() const {
    return fP[4];
  } ///< Retruns Y component of the momentum, fP[4].
  const float32_v &Pz() const {
    return fP[5];
  } ///< Retruns Z component of the momentum, fP[5].
  const float32_v &E() const {
    return fP[6];
  ...
  const int32_v &NDF() const {
    return fNDF;
  } ///< Returns number of decrease of freedom.

  float32_v GetParameter(int i) const {
    return fP[i];
  ...
  float32_v GetCovariance(int i) const {
    ...
    ///< form.
  float32_v GetCovariance(int i, int j) const {
    return Cij(i, j);
  ...
  float32_v GetR() const; ///< Returns distance to the origin of the coordinate
                          ///< system {0,0,0}
  float32_v GetRapidity() const {
    return float32_v(0.5f) * KFPMath::log((fP[6] + fP[5]) / (fP[6] - fP[5]));
  ...
  float32_v GetErrR() const; ///< Returns the error of distance to the origin of
  ...
  mask32_v GetPt(float32_v &Pt,
                 float32_v &SigmaPt) const; //* transverse momentum
  ...
  mask32_v GetDecayLength(float32_v &L,
                          float32_v &SigmaL) const; //* decay length
  mask32_v GetDecayLengthXY(float32_v &L,
                            float32_v &SigmaL) const; //* decay length in XY
  ...
  const float32_v *Parameters() const {
    return fP;
  } ///< Returns pointer to the parameters fP
  const float32_v *CovarianceMatrix() const {
    return fC;
  ...
  void ReconstructMissingMass(const KFParticleSIMD &Daughter,
                              KFParticleSIMD &MotherFiltered,
                              KFParticleSIMD &cDaughterFiltered,
                              float32_v neutralmasshypo);
  ...
  void TransportBz(float32_v Bz, float32_v dS, const float32_v *dsdr,
                   float32_v P[], float32_v C[], float32_v *dsdr1 = 0,
                   float32_v *F = 0, float32_v *F1 = 0,
                   const bool fullC = true) const;
  void TransportBz(float32_v Bz, float32_v dS, float32_v P[]) const;
  void TransportCBM(float32_v dS, const float32_v *dsdr, float32_v P[],
                    float32_v C[], float32_v *dsdr1 = 0, float32_v *F = 0,
                    float32_v *F1 = 0) const;
  void TransportCBM(float32_v dS, float32_v P[]) const;

  void TransportLine(float32_v S, const float32_v *dsdr, float32_v P[],
                     float32_v C[], float32_v *dsdr1 = 0, float32_v *F = 0,
                     float32_v *F1 = 0) const;
  ...
  float32_v GetDStoPointBz(float32_v Bz, const float32_v xyz[3],
                           float32_v dsdr[6], const float32_v *param = 0) const;
  float32_v GetDStoPointBy(float32_v By, const float32_v xyz[3],
                           float32_v dsdr[6]) const;
  ...
  void GetDStoCylinderBz(const float32_v B, const float32_v R,
                         float32_v dS[2]) const;
  ...
  void GetDStoParticle(const KFParticleSIMD &p, float32_v dS[2],
                       float32_v dsdr[4][6]) const;
  void GetDStoParticleFast(const KFParticleSIMD &p, float32_v dS[2]) const;
  void GetDStoParticleLine(const KFParticleSIMD &p, float32_v dS[2],
                           float32_v dsdr[4][6]) const;
  void GetDStoParticleLine(const KFParticleSIMD &p, float32_v dS[2]) const;
  void GetDStoParticleBz(float32_v Bz, const KFParticleSIMD &p, float32_v dS[2],
                         float32_v dsdr[4][6], const float32_v *param1 = 0,
                         const float32_v *param2 = 0) const;
  void GetDStoParticleBz(float32_v Bz, const KFParticleSIMD &p, float32_v dS[2],
                         const float32_v *param1 = 0,
                         const float32_v *param2 = 0) const;
  void GetDStoParticleBy(float32_v B, const KFParticleSIMD &p, float32_v dS[2],
                         float32_v dsdr[4][6]) const;
  void GetDStoParticleBy(float32_v B, const KFParticleSIMD &p,
                         float32_v dS[2]) const;
  void GetDStoParticleB(float32_v B[3], const KFParticleSIMD &p,
                        float32_v dS[2], float32_v dsdr[4][6]) const;
  void GetDStoParticleB(float32_v B[3], const KFParticleSIMD &p,
                        float32_v dS[2]) const;
  void GetDStoParticleCBM(const KFParticleSIMD &p, float32_v dS[2],
                          float32_v dsdr[4][6]) const;
  void GetDStoParticleCBM(const KFParticleSIMD &p, float32_v dS[2]) const;

  void GetDistanceToVertexLine(const KFParticleSIMD &Vertex, float32_v &l,
                               float32_v &dl,
                               mask32_v *isParticleFromVertex = 0) const;
  ...
  mask32_v GetDistanceFromVertexXY(const float32_v vtx[], float32_v &val,
                                   float32_v &err) const;
  mask32_v GetDistanceFromVertexXY(const float32_v vtx[], const float32_v Cv[],
                                   float32_v &val, float32_v &err) const;
  mask32_v GetDistanceFromVertexXY(const KFParticleSIMD &Vtx, float32_v &val,
                                   float32_v &err) const;
#ifdef HomogeneousField
  mask32_v GetDistanceFromVertexXY(const KFPVertex &Vtx, float32_v &val,
                                   float32_v &err) const;
#endif
  ...
  float32_v GetDeviationFromVertexXY(const float32_v v[],
                                     const float32_v Cv[] = 0) const;
  ...
  // @mass - mass of the mother particle (in the case of "Hb -> JPsi" it would
  // be JPsi mass)
  // @*timeErr2 - squared error of the decay time. If timeErr2 = 0 it isn't
  // calculated
  float32_v GetPseudoProperDecayTime(const KFParticleSIMD &primVertex,
                                     const float32_v &mass,
                                     float32_v *timeErr2 = 0) const;

  void GetFieldValue(const float32_v xyz[], float32_v B[]) const;

  void Transport(float32_v dS, const float32_v *dsdr, float32_v P[],
                 float32_v C[], float32_v *dsdr1 = 0, float32_v *F = 0,
                 float32_v *F1 = 0, const bool fullC = true) const;
  void TransportFast(float32_v dS, float32_v P[]) const;

  static void GetArmenterosPodolanski(KFParticleSIMD &positive,
                                      KFParticleSIMD &negative,
                                      float32_v QtAlfa[2]);
  void RotateXY(float32_v angle, float32_v Vtx[3]);
  static void MultQSQt(const float32_v Q[], const float32_v S[],
                       float32_v SOut[], const int kN);

private:
  void GetMeasurement(const KFParticleSIMD &daughter, float32_v m[],
                      float32_v V[], float32_v D[3][3]);
  //* Mass constraint function. is needed for the nonlinear mass constraint and
  // a fit with mass constraint
  void SetMassConstraint(float32_v *mP, float32_v *mC, float32_v mJ[7][7],
                         float32_v mass, mask32_v mask);
  ...
  float32_v fP[8];  ///< Particle parameters { X, Y, Z, Px, Py, Pz, E,
                    ///< S[=DecayLength/P]}.
  ...
  float32_v SumDaughterMass; ///< Sum of the daughter particles masses. Needed
                             ///< to set the constraint on the minimum mass
                             ///< during particle construction.
  float32_v fMassHypo; ///< The mass hypothesis, used for the constraints during
   ...
   **/
  std::vector<int32_v> fDaughterIds; // id of particles it created from. if size
                                     // == 1 then this is id of track.
...
private:
#ifdef HomogeneousField
  static float32_v fgBz; ///< Bz compoment of the magnetic field (is defined in
                         ///< case of #ifdef HomogeneousField)
#endif
#ifdef NonhomogeneousField
...
inline float32_v KFParticleSIMD::GetDStoPoint(const float32_v xyz[3],
                                              float32_v dsdr[6]) const {
...
#ifdef HomogeneousField
inline void KFParticleSIMD::GetFieldValue(const float32_v * /*xyz*/,
                                          float32_v B[]) const {
...
inline void KFParticleSIMD::GetFieldValue(const float32_v xyz[],
   ...
   *field at the given point
   **/
  KFParticleFieldValue mB =
      const_cast<KFParticleFieldRegion &>(fField).Get(xyz[2]);
  B[0] = mB.x;
  B[1] = mB.y;
  B[2] = mB.z;
}
...
inline void KFParticleSIMD::GetDStoParticleFast(const KFParticleSIMD &p,
                                                float32_v dS[2]) const {
  /** Calculates dS = l/p parameters for two particles, where \n
   ** 1) l - signed distance to the DCA point with the other particle;\n
   ** 2) p - momentum of the particleю \n
...
inline void KFParticleSIMD::Transport(float32_v dS, const float32_v *dsdr,
                                      float32_v P[], float32_v C[],
                                      float32_v *dsdr1, float32_v *F,
                                      float32_v *F1, const bool fullC) const {
...
inline void KFParticleSIMD::Load(const KFPTrackVector &track, int index,
                                 const int32_v &pdg) {
  /** Create a particle from a set of consequetive tracks stored in the
   *KFPTrackVector format
  ...
  for (int i = 0; i < 10; i++)
    fField.fField[i] =
        reinterpret_cast<const float32_v &>(track.FieldCoefficient(i)[index]);
#endif
  ...
  float32_v h0 = fP[3] * energyInv, h1 = fP[4] * energyInv,
            h2 = fP[5] * energyInv;
