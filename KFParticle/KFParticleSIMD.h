...
#error "Both HomogeneousField and NonhomogeneousField are defined — choose only one!"
#endif
...
class KFParticleSIMD {
 ...
 public:
  ...
  { return Cij(i, j); }  ///< Returns C[i,j] element of the covariance matrix.
  ...
  { return float32_v(0.5f) * KFPMath::log((fP[6] + fP[5]) / (fP[6] - fP[5])); }  ///< Returns rapidity of the particle
  ...
  void SetId(int32_v id) { fId = id; }  ///< Sets the Id of the particle. After the construction of a particle
  ...
  void ReconstructMissingMass(const KFParticleSIMD& Daughter, KFParticleSIMD& MotherFiltered,
  ...
  void Construct(const KFParticleSIMD* vDaughters[], int nDaughters, const KFParticleSIMD* ProdVtx = 0,
                 Float_t Mass = -1);
  ...
  void TransportBz(float32_v Bz, float32_v dS, const float32_v* dsdr, float32_v P[], float32_v C[],
                   float32_v* dsdr1 = 0, float32_v* F = 0, float32_v* F1 = 0, const bool fullC = true) const;
  void TransportBz(float32_v Bz, float32_v dS, float32_v P[]) const;
  void TransportCBM(float32_v dS, const float32_v* dsdr, float32_v P[], float32_v C[], float32_v* dsdr1 = 0,
                    float32_v* F = 0, float32_v* F1 = 0) const;
  void TransportCBM(float32_v dS, float32_v P[]) const;

  void TransportLine(float32_v S, const float32_v* dsdr, float32_v P[], float32_v C[], float32_v* dsdr1 = 0,
                     float32_v* F = 0, float32_v* F1 = 0) const;
...
#ifdef HomogeneousField
  float32_v GetDStoPointXY(const float32_v xyz[2]) const { return GetDStoPointXYBz(GetFieldAlice(), xyz); }
  //* Get dS to a certain space point
  void GetDStoCylinder(const float32_v R, float32_v dS[2]) const { GetDStoCylinderBz(GetFieldAlice(), R, dS); }
#endif
  ...
  void GetDStoParticleBz(float32_v Bz, const KFParticleSIMD& p, float32_v dS[2], float32_v dsdr[4][6],
                         const float32_v* param1 = 0, const float32_v* param2 = 0) const;
  void GetDStoParticleBz(float32_v Bz, const KFParticleSIMD& p, float32_v dS[2], const float32_v* param1 = 0,
  ...
  void GetDistanceToVertexLine(const KFParticleSIMD& Vertex, float32_v& l, float32_v& dl,
  ...
  float32_v GetDeviationFromVertex(const float32_v v[], const float32_v Cv[] = 0) const;
  ...
  // calculated
  float32_v GetPseudoProperDecayTime(const KFParticleSIMD& primVertex, const float32_v& mass,
                                     float32_v* timeErr2 = 0) const;

  void GetFieldValue(const float32_v xyz[], float32_v B[]) const;

  void Transport(float32_v dS, const float32_v* dsdr, float32_v P[], float32_v C[], float32_v* dsdr1 = 0,
                 float32_v* F = 0, float32_v* F1 = 0, const bool fullC = true) const;
...
inline float32_v KFParticleSIMD::GetErrX() const { return sqrt(abs(GetCovariance(0, 0))); }

inline float32_v KFParticleSIMD::GetErrY() const { return sqrt(abs(GetCovariance(1, 1))); }

inline float32_v KFParticleSIMD::GetErrZ() const { return sqrt(abs(GetCovariance(2, 2))); }

inline float32_v KFParticleSIMD::GetErrPx() const { return sqrt(abs(GetCovariance(3, 3))); }

inline float32_v KFParticleSIMD::GetErrPy() const { return sqrt(abs(GetCovariance(4, 4))); }

inline float32_v KFParticleSIMD::GetErrPz() const { return sqrt(abs(GetCovariance(5, 5))); }

inline float32_v KFParticleSIMD::GetErrE() const { return sqrt(abs(GetCovariance(6, 6))); }

inline float32_v KFParticleSIMD::GetErrS() const { return sqrt(abs(GetCovariance(7, 7))); }
...
inline void KFParticleSIMD::Transport(float32_v dS, const float32_v* dsdr, float32_v P[], float32_v C[],
