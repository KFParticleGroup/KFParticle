...
public:
  KFParticlePVReconstructor()
      : fParticles(0), fNParticles(0), fWeight(0.f), fBeamLine(),
        fIsBeamLine(0), fClusters(0), fPrimVertices(0),
        fChi2CutPreparation(100), fChi2Cut(16){};
  ~KFParticlePVReconstructor() {}
...
private:
  KFParticlePVReconstructor &operator=(
      KFParticlePVReconstructor
  ...
  std::vector<KFParticle>
      fParticles;  ///< Array of the input particles constructed from tracks.
  int fNParticles; ///< Number of the input particles.

  std::array<float, 3> fTarget = {0.f};

  std::vector<float>
      fWeight; ///< Vector with weights of each track, the weight is defined in
