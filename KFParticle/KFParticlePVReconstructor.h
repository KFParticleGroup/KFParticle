...
class KFParticlePVReconstructor {
 public:
  ...
  { return fPrimVertices.size(); }  ///< Returns number of the found candidates for the primary vertex.
  KFParticle& GetPrimVertex(int iPV = 0)
  { return fPrimVertices[iPV]; }  ///< Returns primary vertex candidate in KFParticle with index "iPV".
  KFVertex& GetPrimKFVertex(int iPV = 0)
  { return fPrimVertices[iPV]; }  ///< Returns primary vertex candidate in KFVertex with index "iPV".
  std::vector<int>& GetPVTrackIndexArray(int iPV = 0)
  { return fClusters[iPV].fTracks; }  ///< Returns vector with track indices from a cluster with index "iPV".
  ...
  void SetBeamLine(KFParticle& p)
  {
    fBeamLine   = p;
    fIsBeamLine = 1;
  }  ///< Sets the beam line position and direction, sets corresponding flag to
  ///< "true".
  bool IsBeamLine() const { return fIsBeamLine; }  ///< Check if the beam line is set.

  /** Adds externally found primary vertex to the list together with the cluster
   *of
  ...
  void CleanPV()
  {
    fClusters.clear();
    fPrimVertices.clear();
  }  ///< Clean vectors with primary vertex candidates and corresponding
  ///< clusters.

  /** \brief Sets cut fChi2Cut on chi2-deviation of primary tracks from the
   *vertex candidate to "chi2"
  ...
  void SetTargetPosition(const std::array<float, 3>& target) { fTarget = target; }
  const std::array<float, 3> GetTargetPosition() { return fTarget; }

 private:
  KFParticlePVReconstructor&
  operator=(KFParticlePVReconstructor&);  ///< Is not defined. Deny copying of the objects of this class.
  KFParticlePVReconstructor(
    KFParticlePVReconstructor&);  ///< Is not defined. Deny copying of the objects of this class.
  ...
  std::vector<float> fWeight;  ///< Vector with weights of each track, the weight is defined in
                               ///< KFParticlePVReconstructor::Init().
   ...
   ** @date 05.02.2019
   ** @version 1.0
   ** Contains a list of track and initial approximation for the vertex
   *position,
  ...
  std::vector<KFParticleCluster> fClusters;  ///< Vector with clusters to be used
                                             ///< for fit of a primary vertex.
  std::vector<KFVertex> fPrimVertices;       ///< Vector with reconstructed candidates
                                             ///< for a primary vertex.

  float fChi2CutPreparation;  ///< A soft cut on the chi2-deviation which is used
                              ///< to form a cluster.
  float fChi2Cut;             ///< Cut on the chi2-deviation of the tracks to the primary
                              ///< vertex \see KFVertex::ConstructPrimaryVertex(), where it
                              ///< is used.
};  // class KFParticlePVReconstructor
