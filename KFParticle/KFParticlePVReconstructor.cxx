...
void KFParticlePVReconstructor::Init(KFPTrackVector* tracks, int nParticles)
{
  ...
  for (int iIter = 0; iIter < 3; iIter++) {
    ...
    for (int iTr = 0; iTr < fNParticles; iTr++) {
      float ds      = 0.f;
      float dsdr[6] = {0.f};
      if (iIter > 0) { ds = fParticles[iTr].GetDStoPoint(pvEstimationTr, dsdr); }
      ...
      for (int iComp = 0; iComp < 3; iComp++) {
        ...
        if (C[iComp] > 16 * V[iComp]) { K = 1.f - V[iComp] / C[iComp]; }
  ...
  for (int iP = 0; iP < fNParticles; iP++) {
    ...
    fWeight[iP] = fParticles[iP].CovarianceMatrix()[0] + fParticles[iP].CovarianceMatrix()[2]
                  + fParticles[iP].CovarianceMatrix()[5];

    if (fWeight[iP] > 0.f) { fWeight[iP] = 1.f / sqrt(fWeight[iP]); }
    else {
      fWeight[iP] = -100;
    }
...
void KFParticlePVReconstructor::FindPrimaryClusters(int cutNDF)
{
  ...
  for (int iTr = 0; iTr < fNParticles; iTr++) { (*notUsedTracksPtr)[iTr] = iTr; }
  ...
  while (nNotUsedTracks > 0) {
    ...
    for (unsigned short int iTr = 0; iTr < nNotUsedTracks; iTr++) {
      ...
      if ((chi2deviation < fChi2CutPreparation && chi2deviation >= 0 && fWeight[curTrack] > -1.f)
          || curTrack == bestTrack) {
        for (int iP = 0; iP < 3; iP++) { rVertex[iP] += fWeight[curTrack] * fParticles[curTrack].Parameters()[iP]; }
    ...
    for (int iP = 0; iP < 3; iP++) { cluster.fP[iP] = rVertex[iP] / weightVertex; }

    for (int iC = 0; iC < 6; iC++) { cluster.fC[iC] = covVertex[iC] / (weightVertex * weightVertex); }
    ...
    for (int iP = 0; iP < nPrimCand; iP++) { pParticles[iP] = &fParticles[cluster.fTracks[iP]]; }
    ...
    if (fNParticles > 1) {
      ...
      for (int iFl = 0; iFl < nPrimCand; iFl++) { vFlags[iFl] = true; }
      ...
      for (int iP = 0; iP < nTracks; iP++) {
        ...
        if (vFlags[iP]) { clearClusterInd.push_back(cluster.fTracks[iP]); }
        else {
