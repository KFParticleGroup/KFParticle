...
/** @class KFMCTrack
 ** @brief A class for storage of the Monte Carlo simulated track in the
 *cartesian parametrisation.
 ** @author  M.Zyzak, I.Kisel
 ** @date 05.02.2019
 ** @version 1.0
 **
 ** A track is described with the parameters { X, Y, Z, Px, Py, Pz, q/P }.
 *Parameters are stored at
 ** the origin position. Class also contains Id of the mother track, PDG code
 *for the current track,
 ** number of Monte Carlo points produced by the simulation engine at the
 *detector stations,
 ** number of Monte Carlo points produced at the precise detectors (like MVD in
 *CBM, HFT in STAR,
 ** ITS in ALICE, etc.). It also has a flag showing if track was found by the
 *reconstruction procedure
 ** for efficiency calculation, and a flag showing if track was out of
 *acceptance.
 **/
...
public:
  KFMCTrack()
      : fMotherId(-1), fPDG(0), fNMCPoints(0), fNMCPixelPoints(0),
        fIsReconstructed(0), fIsOutOfDetector(0){};

  int MotherId() const {
    return fMotherId;
  } ///< Returns a uniqueue Id of the mother track or primary vertex
    ///< KFMCTrack::fMotherId.
  int PDG() const {
    return fPDG;
  ...
  float P() const {
    return sqrt(fPar[3] * fPar[3] + fPar[4] * fPar[4] + fPar[5] * fPar[5]);
  } ///< Returns momentum of the track.
  float Pt() const {
    return sqrt(fPar[3] * fPar[3] + fPar[4] * fPar[4]);
  ...
  int NMCPixelPoints() const {
    return fNMCPixelPoints;
  } ///< Returns number of MC points at the precise detectors
    ///< KFMCTrack::fNMCPixelPoints.
  bool IsReconstructed() const {
    return fIsReconstructed;
  } ///< Returns a flag showing if track was found by the reconstruction
    ///< procedure.
  bool IsOutOfDetector() const {
    return fIsOutOfDetector;
  ...
  void SetMotherId(int v) {
    fMotherId = v;
  } ///< Sets a unique id of the mother track if track is secondary or primary
...
protected:
  int fMotherId;  ///< Index of the mother track in tracks array. If track is
                  ///< produced at the primary vertex (PV) negative value with
                  ///< the PV Id is assigned.
  int fPDG;       ///< The PDG code of the current Monte Carlo track.
  float fPar[7];  ///< Parameters of the track: { X, Y, Z, Px, Py, Pz, q/P },
                  ///< where "q" is its charge.
  int fNMCPoints; ///< Total number of Monte Carlo points produced by the
                  ///< simulation engine at the detector stations.
  int fNMCPixelPoints; ///< Number of Monte Carlo points produced at the precise
                       ///< detectors (like MVD in CBM, HFT in STAR, ITS in
                       ///< ALICE, etc.).

  bool fIsReconstructed; ///< A flag showing if track was found by the
                         ///< reconstruction procedure. Is required for correct
                         ///< efficiency calculation.
  bool
      fIsOutOfDetector; ///< A flag showing if track was out of acceptance. Is
                        ///< required for correct calculation of the acceptance.
};
