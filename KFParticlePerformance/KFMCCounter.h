...
/** @class KFMCCounter
 ** @brief A helper structure to store information on the number of
 *reconstructed and Monte Carlo particles for efficiency calculation.
 ** @author  M.Zyzak, I.Kisel
 ** @date 05.02.2019
 ** @version 1.0
 **
 ** The class is used to calculate reconstruction efficiency and ratios of a
...
struct KFMCCounter {
  ...
  KFMCCounter(int nCounters)
    : NCounters(nCounters)
    , counters(nCounters, T(0)) {}  ///< Constructs the object with the set of counters "nCounters".
  ...
  /** Operator adds all counters from object "a" to the current object. Returns
   * the current object. */
  KFMCCounter& operator+=(KFMCCounter& a)
  {
    if (NCounters != a.NCounters) {
      std::cout << " KFMCCounter: Error. Addition of counters of different sizes: " << NCounters << " " << a.NCounters
                << std::endl;
    }
    else {
      for (int iC = 0; iC < NCounters; iC++) { counters[iC] += a.counters[iC]; }
    }
    return *this;
  };
  /** Operator adds all counters from object "a" to the current object, result
   * is stored to the temporary object. Returns the temporary object. */
  KFMCCounter operator+(KFMCCounter& a)
  {
  ...
  /** Operator divides all counters from the current object to the counters from
   * object "a", result is stored to the temporary object. Returns the temporary
   * object. */
  ...
  KFMCCounter<double> operator/(KFMCCounter<T2>& a)
  {
    ...
    if (NCounters != a.NCounters) {
      ...
      for (int iC = 0; iC < NCounters; iC++) { b.counters[iC] = Div(counters[iC], a.counters[iC]); }
    }
    return b;
  }
  /** Operator divides all counters from the current object to the value "a",
   * result is stored to the temporary object. Returns the temporary object. */
  template<typename T2>
  KFMCCounter<T2> operator/(double a)
  {
    KFMCCounter<T2> b(NCounters);
    for (int iC = 0; iC < NCounters; iC++) { b.counters[iC] = (T2) Div(counters[iC], a); }
    return b;
  }
  ...
  friend std::fstream& operator<<(std::fstream& strm, const KFMCCounter<T>& a)
  {
    ...
    for (unsigned int iV = 0; iV < a.counters.size(); iV++) { strm << a.counters[iV] << " "; }
    strm << std::endl;
    return strm;
  }
