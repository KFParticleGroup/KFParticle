...
KFParticlePerformanceBase::KFParticlePerformanceBase()
  : fParteff()
  , fPVeff()
  , fPVeffMCReconstructable()
  , outfileName()
  , histodir(0)
  , fNEvents(0)
  , fStoreMCHistograms(1)
  , fStorePrimSecHistograms(1)
  , fStoreZRHistograms(1)
  , fStore3DEfficiency(0)
  , fHistoDir(0)
{
  ...
  for (int iParticle = 0; iParticle < KFPartEfficiencies::nParticles; iParticle++) {
    for (int iQA = 0; iQA < nDSToParticleQA; iQA++) { hDSToParticleQA[iParticle][iQA] = nullptr; }
  }
  ...
  for (int iParticle = 0; iParticle < KFPartEfficiencies::nParticles; iParticle++) {
    for (int iHisto = 0; iHisto < nHistoPartParam3D; iHisto++) { hPartParam3D[0][iParticle][iHisto] = nullptr; }
  }

  for (int iParticle = 0; iParticle < KFPartEfficiencies::nParticles; iParticle++) {
    for (int iEffSet = 0; iEffSet < 3; iEffSet++) {
      for (int iEff = 0; iEff < nPartEfficiency; iEff++) { hPartEfficiency[iParticle][iEffSet][iEff] = nullptr; }
    }
  }

  for (int iParticle = 0; iParticle < KFPartEfficiencies::nParticles; iParticle++) {
    for (int iEffSet = 0; iEffSet < 3; iEffSet++) {
      for (int iEff = 0; iEff < nPartEfficiency2D; iEff++) { hPartEfficiency2D[iParticle][iEffSet][iEff] = nullptr; }
    }
  }

  for (int iParticle = 0; iParticle < KFPartEfficiencies::nParticles; iParticle++) {
    for (int iEffSet = 0; iEffSet < 4; iEffSet++) { hPartEfficiencyMulti[iParticle][iEffSet] = nullptr; }
  }

  for (int iEffSet = 0; iEffSet < 2; iEffSet++) {
    for (int iHistoPV = 0; iHistoPV < nHistosPV; iHistoPV++) { hPVFitQa[iEffSet][iHistoPV] = nullptr; }
  }
  ...
  for (int iParam = 0; iParam < nHistosPVParam2D; iParam++) { hPVParam2D[iParam] = nullptr; }

  for (int iFitQA = 0; iFitQA < nFitPVTracksQA; iFitQA++) { hFitPVTracksQA[iFitQA] = nullptr; }

  for (int iTP = 0; iTP < nHistosTP; iTP++) { hTrackParameters[iTP] = nullptr; }
...
void KFParticlePerformanceBase::CreateHistos(std::string histoDir, TDirectory* outFile, std::map<int, bool> decays)
{
  ...
  }
  {
    ...
    for (int iPart = 0; iPart < fParteff.nParticles; ++iPart) {
      if (!(decays.empty())
          && (iPart < fParteff.fFirstStableParticleIndex || iPart > fParteff.fLastStableParticleIndex)) {
      ...
      gDirectory->cd(fParteff.partName[iPart].data());
      {
        if ((iPart >= 48 && iPart <= 92) || (iPart == 13 || iPart == 14 || iPart == 37 || iPart == 38)) {
          ...
          gDirectory->cd("Daughters");
          {
            for (uint32_t iDaughter = 0; iDaughter < fParteff.partDaughterPdg[iPart].size(); iDaughter++) {
              ...
              {
                hPartDaughterPTheta[iPart][iDaughter] =
                  new TH2F("P vs #theta", GetDirectoryPath() + TString("P vs #theta"), 1000., 0., 10., 1000., 0, M_PI);
                hPartDaughterPhi[iPart][iDaughter] =
                  new TH1F("#phi", GetDirectoryPath() + TString("#phi"), 1000., -M_PI, M_PI);
              }
              gDirectory->cd("..");  //Daughters
            }
          }
        ...
        if (fStoreMCHistograms) {
          ...
          gDirectory->cd("DaughtersQA");
          {
            ...
            for (int iH = 0; iH < nFitQA / 2; iH++) {
              hFitDaughtersQA[iPart][iH] =
                new TH1F((res + parName[iH]).Data(), (GetDirectoryPath() + res + parName[iH]).Data(), nBins, -xMax[iH],
                         xMax[iH]);
              hFitDaughtersQA[iPart][iH + 8] =
          ...
          gDirectory->cd("DSToParticleQA");
          {
            ...
            for (int iH = 0; iH < 3; iH++) {
              hDSToParticleQA[iPart][iH] =
                new TH1F((res + parName[iH]).Data(), (GetDirectoryPath() + res + parName[iH]).Data(), nBins, -xMax[iH],
                         xMax[iH]);
              hDSToParticleQA[iPart][iH + 3] =
                new TH1F((pull + parName[iH]).Data(), (GetDirectoryPath() + pull + parName[iH]).Data(), nBins, -6, 6);
            }
        ...
        std::cout << "KFParticlePerformanceBase::CreateHistos : gDirectory->cd(\"Parameters\")" << std::endl;
        {
          ...
          if (fStoreMCHistograms) {
            ...
            bool plotPrimaryHistograms = abs(fParteff.partPDG[iPart]) == 310 || abs(fParteff.partPDG[iPart]) == 3122
                                         || abs(fParteff.partPDG[iPart]) == 22 || abs(fParteff.partPDG[iPart]) == 111
                                         || abs(fParteff.partPDG[iPart]) == 3312
                                         || abs(fParteff.partPDG[iPart]) == 3334;

            bool plotSecondaryHistograms = abs(fParteff.partPDG[iPart]) == 310 || abs(fParteff.partPDG[iPart]) == 3122
                                           || abs(fParteff.partPDG[iPart]) == 22 || abs(fParteff.partPDG[iPart]) == 111;

            if (fStorePrimSecHistograms && plotPrimaryHistograms) {
              std::cout
                << "KFParticlePerformanceBase::CreateHistos : if(fStorePrimSecHistograms && plotPrimaryHistograms)"
                << std::endl;
              gDirectory->mkdir("Primary");
              gDirectory->cd("Primary");
              {
                CreateParameterSubfolder("NoConstraint (1C-Fit)", hPartParamPrimary, hPartParam2DPrimary,
                                         hFitQANoConstraint, iPart, true);
                CreateParameterSubfolder("MassConstraint (2C-Fit)", hPartParamPrimaryMass, hPartParam2DPrimaryMass,
                                         hFitQAMassConstraint, iPart, true);
                CreateParameterSubfolder("PVConstraint (3C-Fit)", hPartParamPrimaryTopo, hPartParam2DPrimaryTopo,
                                         hFitQATopoConstraint, iPart, true);
                CreateParameterSubfolder("PVMassConstraint (4C-Fit)", hPartParamPrimaryTopoMass,
                                         hPartParam2DPrimaryTopoMass, hFitQATopoMassConstraint, iPart, true);
              }
              gDirectory->cd("..");  // particle directory / Parameters
            }
            ...
            if (fStorePrimSecHistograms && plotSecondaryHistograms) {
              std::cout
                << "KFParticlePerformanceBase::CreateHistos : if(fStorePrimSecHistograms && plotSecondaryHistograms)"
                << std::endl;
              ...
              gDirectory->cd("Secondary");
              {
                CreateParameterSubfolder("NoConstraint (1C-Fit)", hPartParamSecondary, hPartParam2DSecondary, 0, iPart,
                                         true);
                CreateParameterSubfolder("MassConstraint (2C-Fit)", hPartParamSecondaryMass, hPartParam2DSecondaryMass,
                                         0, iPart, true);
              }
              gDirectory->cd("..");  // particle directory / Parameters
            }
    ...
    gDirectory->cd("PrimaryVertexQA");
    {
      struct {
        ...
        Int_t n;
        Double_t l, r;
      } Table[nHistosPV] = {{"PVResX", "x_{rec}-x_{mc}, cm", 100, -0.1f, 0.1f},
      ...
      TString parName[nHistosPVParam]     = {"x",       "y",         "z",        "r",    "Ntracks",
                                             "Chi2",    "NDF",       "Chi2NDF",  "prob", "PVpurity",
                                             "ghostTr", "triggerTr", "pileupTr", "bgTr", "dzSamePV"};
      TString parAxisName[nHistosPVParam] = {"x [cm]",
                                             "y [cm]",
                                             "z [cm]",
                                             "r [cm]",
                                             "N tracks",
                                             ...
                                             "bg tracks [%]",
                                             "dz [cm]"};
      int nBins[nHistosPVParam]           = {1000, 1000, 1000, 1000, 1001, 10000, 1001, 10000,
                                             100,  102,  102,  102,  102,  102,   1000};
      ...
      float xMax[nHistosPVParam] = {10., 10.,  205., 10,   1000.5, 1000., 1000.5, 1000.,
                                    1.,  1.01, 1.01, 1.01, 1.01,   1.01,  100.};
      ...
      for (int iH = 0; iH < nHistosPVParam; iH++) {
        hPVParam[iH] =
          new TH1F(parName[iH].Data(), (GetDirectoryPath() + parName[iH]).Data(), nBins[iH], xMin[iH], xMax[iH]);
        hPVParam[iH]->GetXaxis()->SetTitle(parAxisName[iH].Data());
      }

      for (int iH = 0; iH < nHistosPVParam2D; iH++) {
        hPVParam2D[iH] = new TH2F(parName2D[iH].Data(), (GetDirectoryPath() + parName2D[iH]).Data(), nBinsX2D[iH],
                                  xMin2D[iH], xMax2D[iH], nBinsY2D[iH], yMin2D[iH], yMax2D[iH]);
      ...
      {
        TString effName[nPVefficiency] = {"effVsNMCPVTracks", "effVsNMCPV", "effVsNMCTracks",
                                          "effVsNPVTracks",   "effVsNPV",   "effVsNTracks"};
        ...
        gDirectory->cd("Signal");
        {
          for (int iH = 0; iH < nPVefficiency; iH++) {
            hPVefficiency[0][iH] = new TProfile(effName[iH].Data(), (GetDirectoryPath() + effName[iH]).Data(),
                                                nBinsEff[iH], xMinEff[iH], xMaxEff[iH]);
          }
        }
        gDirectory->cd("..");  //L1
        ...
        {
          for (int iH = 0; iH < nPVefficiency; iH++) {
            hPVefficiency[1][iH] = new TProfile(effName[iH].Data(), (GetDirectoryPath() + effName[iH].Data()),
                                                nBinsEff[iH], xMinEff[iH], xMaxEff[iH]);
          }
        }
        gDirectory->cd("..");  //L1
        ...
        {
          for (int iH = 0; iH < nPVefficiency; iH++) {
            hPVefficiency[2][iH] = new TProfile(effName[iH].Data(), (GetDirectoryPath() + effName[iH].Data()),
                                                nBinsEff[iH], xMinEff[iH], xMaxEff[iH]);
          }
        }
        gDirectory->cd("..");  //L1
        ...
        {
          for (int iH = 0; iH < nPVefficiency; iH++) {
            hPVefficiency[3][iH] = new TProfile(effName[iH].Data(), (GetDirectoryPath() + effName[iH].Data()),
                                                nBinsEff[iH], xMinEff[iH], xMaxEff[iH]);
          }
        }
        gDirectory->cd("..");  //L1
      ...
      gDirectory->cd("PVTracksQA");
      {
        ...
        for (int iH = 0; iH < nFitPVTracksQA / 2; iH++) {
          hFitPVTracksQA[iH] =
            new TH1F((resTrPV + parNameTrPV[iH]).Data(), (GetDirectoryPath() + resTrPV + parNameTrPV[iH]).Data(),
                     nBinsTrPV, -xMaxTrPV[iH], xMaxTrPV[iH]);
          hFitPVTracksQA[iH + nFitPVTracksQA / 2] =
            new TH1F((pullTrPV + parNameTrPV[iH]).Data(), (GetDirectoryPath() + pullTrPV + parNameTrPV[iH]).Data(),
                     nBinsTrPV, -6, 6);
        }
      }
      gDirectory->cd("..");  //L1
        ...
        gDirectory->cd("FitQA");
        {
          ...
          gDirectory->cd("FitQAvcNMCPVTracks");
          {
            for (int iHPV = 0; iHPV < nHistosPV - 1; ++iHPV) {
              hPVFitQa2D[0][0][iHPV] =
                new TH2F(Table[iHPV].name.Data(), (GetDirectoryPath() + Table[iHPV].title).Data(), 500, 0., 5000.,
    ...
    gDirectory->cd("TrackParameters");
    {
      ...
      for (int iPart = 0; iPart < KFPartEfficiencies::nParticles; iPart++) {
        ...
        hTrackParameters[iPart] =
          new TH1F(chi2NamePart.Data(), (GetDirectoryPath() + chi2NamePart).Data(), 1000, 0, 100);
      }
      hTrackParameters[KFPartEfficiencies::nParticles] =
        new TH1F("Chi2Prim_total", (GetDirectoryPath() + TString("Chi2Prim_total")), 1000, 0, 100);
      hTrackParameters[KFPartEfficiencies::nParticles + 1] =
        new TH1F("Chi2Prim_prim", (GetDirectoryPath() + TString("Chi2Prim_prim")), 1000, 0, 100);
      hTrackParameters[KFPartEfficiencies::nParticles + 2] =
        new TH1F("Chi2Prim_sec", (GetDirectoryPath() + TString("Chi2Prim_sec")), 1000, 0, 100);
      hTrackParameters[KFPartEfficiencies::nParticles + 3] =
        new TH1F("Chi2Prim_ghost", (GetDirectoryPath() + TString("Chi2Prim_ghost")), 1000, 0, 100);

      hTrackParameters[KFPartEfficiencies::nParticles + 4] =
        new TH1F("ProbPrim_total", (GetDirectoryPath() + TString("ProbPrim_total")), 10000, 0, 1);
      hTrackParameters[KFPartEfficiencies::nParticles + 5] =
        new TH1F("ProbPrim_prim", (GetDirectoryPath() + TString("ProbPrim_prim")), 10000, 0, 1);
      hTrackParameters[KFPartEfficiencies::nParticles + 6] =
        new TH1F("ProbPrim_sec", (GetDirectoryPath() + TString("ProbPrim_sec")), 10000, 0, 1);
      hTrackParameters[KFPartEfficiencies::nParticles + 7] =
        new TH1F("ProbPrim_ghost", (GetDirectoryPath() + TString("ProbPrim_ghost")), 10000, 0, 1);
    }
    gDirectory->cd("..");  //particle directory
    curdir->cd();
  }
...
void KFParticlePerformanceBase::CreateFitHistograms(TH1F* histo[nFitQA], int iPart)
{
  ...
  gDirectory->cd("FitQA");
  {
    ...
    if (iPart > 63 && iPart < 75) {
      for (int iMult = 3; iMult < nFitQA / 2; iMult++) { mult[iMult] = 3; }
    }
    if (iPart > 45 && iPart < 64) {
#ifdef CBM
      for (int iMult = 0; iMult < 3; iMult++) { mult[iMult] = 0.03; }
      for (int iMult = 3; iMult < nFitQA / 2; iMult++) { mult[iMult] = 3; }
#else
      mult[2] = 0.1;
      for (int iMult = 3; iMult < nFitQA / 2; iMult++) { mult[iMult] = 10; }
      mult[5] = 2;
      mult[6] = 2;
#endif
    }
    if (iPart == 44 || iPart == 45) {
      ...
      mult[1] = 0.5;
      mult[2] = 0.15;
      for (int iMult = 3; iMult < nFitQA / 2; iMult++) { mult[iMult] = 4; }
    }

    for (int iH = 0; iH < nFitQA / 2; iH++) {
      histo[iH] = new TH1F((res + parName[iH]).Data(), (GetDirectoryPath() + res + parName[iH]).Data(), nBins,
                           -mult[iH] * xMax[iH], mult[iH] * xMax[iH]);
                                                           ...
                                                           THnSparseF* histoN[4], int iPart)
{
  ...
  {  //vs p, pt, y, z, c*tau, decay length, l, r
    TString partNameEff[nPartEfficiency]     = {"EffVsP",  "EffVsPt", "EffVsY", "EffVsZ", "EffVsCT",
                                                "EffVsDL", "EffVsL",  "EffVsR", "EffVsMt"};
    TString partAxisNameEff[nPartEfficiency] = {
      "p [GeV/c]",         "p_{t} [GeV/c]", "y",        "z [cm]",           "Life time c#tau [cm]",
      "Decay length [cm]", "L [cm]",        "Rxy [cm]", "m_{t} [GeV/c^{2}]"};
#ifdef CBM
...
#endif
    TString effTypeName[3] = {"All particles", "Reconstructable daughters", "Reconstructed daughters"};
    ...
    for (int iEff = 0; iEff < 3; iEff++) {
      ...
      gDirectory->cd(effTypeName[iEff].Data());
      {
        for (int iH = 0; iH < nPartEfficiency; iH++) {
          histo[iEff][iH] = new TProfile(partNameEff[iH].Data(), (GetDirectoryPath() + partAxisNameEff[iH]).Data(),
                                         nBinsEff[iH], xMinEff[iH], xMaxEff[iH]);
        ...
        histo2[iEff][0] =
          new TProfile2D("EffVsPtVsY", (GetDirectoryPath() + partAxisNameEff[2] + partAxisNameEff[1]).Data(),
        ...
        histo2[iEff][1] =
          new TProfile2D("EffVsMtVsY", (GetDirectoryPath() + partAxisNameEff[2] + partAxisNameEff[8]).Data(),
        ...
        if (IsCollectDalitz(iPart)) {
          ...
          histo2[iEff][2] = new TProfile2D(
            "EffDalitz1", (GetDirectoryPath() + fParteff.partName[index1] + fParteff.partName[index2]).Data(), nBinsM12,
            m12Min, m12Max, nBinsM23, m23Min, m23Max);
          ...
          histo2[iEff][3] = new TProfile2D(
            "EffDalitz2", (GetDirectoryPath() + fParteff.partName[index2] + fParteff.partName[index3]).Data(), nBinsM13,
            m13Min, m13Max, nBinsM23, m23Min, m23Max);
          ...
          histo2[iEff][4] = new TProfile2D(
            "EffDalitz3", (GetDirectoryPath() + fParteff.partName[index1] + fParteff.partName[index3]).Data(), nBinsM12,
            m12Min, m12Max, nBinsM13, m13Min, m13Max);
    ...
    if (fStore3DEfficiency) {
      ...
      gDirectory->cd("Multidimensional");
      {
        ...
        histoN[0] =
          new THnSparseF("Acceptance_reco", GetDirectoryPath() + "Acceptance reco", nDimensions, nBins, xMin, xMax);
        ...
        histoN[1] =
          new THnSparseF("Acceptance_mc", GetDirectoryPath() + "Acceptance mc", nDimensions, nBins, xMin, xMax);
        ...
        histoN[2] = new THnSparseF("Cuts_reco", GetDirectoryPath() + "Cuts reco", nDimensions2, nBins2, xMin2, xMax2);
        ...
        histoN[3] = new THnSparseF("Cuts_mc", GetDirectoryPath() + "Cuts mc", nDimensions2, nBins2, xMin2, xMax2);
        ...
        histoN[0] = new THnSparseF("Acceptance_reco", GetDirectoryPath() + "phi-theta-p-pt-eta reco", nDimensions,
                                   nBins, xMin, xMax);
        histoN[0]->GetAxis(0)->SetTitle("#phi");
        histoN[0]->GetAxis(1)->SetTitle("#theta");
        histoN[0]->GetAxis(2)->SetTitle("p [GeV/c]");
        ...
        histoN[1] =
          new THnSparseF("Acceptance_mc", GetDirectoryPath() + "phi-theta-p-pt-eta mc", nDimensions, nBins, xMin, xMax);
        histoN[1]->GetAxis(0)->SetTitle("#phi");
        histoN[1]->GetAxis(1)->SetTitle("#theta");
        histoN[1]->GetAxis(2)->SetTitle("p [GeV/c]");
        ...
        histoN[2] =
          new THnSparseF("Cuts_reco", GetDirectoryPath() + "pt-y-p-DL-ldl reco", nDimensions2, nBins2, xMin2, xMax2);
        ...
        histoN[3] =
          new THnSparseF("Cuts_mc", GetDirectoryPath() + "pt-y-p-DL-ldl mc", nDimensions2, nBins2, xMin2, xMax2);
...
void KFParticlePerformanceBase::CreateParameterHistograms(
  ...
  TH3F* histoParameters3D[KFPartEfficiencies::nParticles][nHistoPartParam3D], int iPart, bool drawZR)
{
   ...
   **/
  std::cout << "KFParticlePerformanceBase::CreateParameterHistograms : nHistoPartParam = " << nHistoPartParam
            << std::endl;
#ifdef CBM
  TString parName[nHistoPartParam] = {"M",   "p", "p_{t}", "y", "DecayL", "c#tau", "chi2ndf", "prob",  "#theta",
                                      "phi", "X", "Y",     "Z", "R",      "L",     "l/dl",    "m_{t}", "Multiplicity"};
#else
  TString parName[nHistoPartParam] = {
    "M", "p", "p_{t}", "y",     "DecayL",       "c#tau", "chi2ndf", "prob", "#theta", "phi", "X",   "Y",  "Z",
    "R", "L", "l/dl",  "m_{t}", "Multiplicity", "dX",    "dY",      "dZ",   "dPx",    "dPy", "dPz", "dE", "dM"};
#endif
  ...
  TString parName3D[nHistoPartParam3D] = {"y-p_{t}-M",       "y-m_{t}-M", "centrality-pt-M", "centrality-y-M",
                                          "centrality-mt-M", "ct-pt-M",   "dalitz",          "dalitz2",
                                          "dalitz3",         "dalitzM2",  "dalitz2M2",       "dalitz3M2"};
  ...
  for (int iParam = 0; iParam < nHistoPartParam; iParam++) {
    ...
    if (iParam < nHistoPartParam2D) { parTitle2D[iParam] = path + parName2D[iParam]; }
    if (iParam < nHistoPartParam3D) { parTitle3D[iParam] = path + parName3D[iParam]; }
  }
#ifdef CBM
  TString parAxisName[nHistoPartParam] = {"m [GeV/c^{2}]",
                                          "p [GeV/c]",
                                          "p_{t} [GeV/c]",
                                          "y",
                                          "Decay length [cm]",
  ...
  float xMin[nHistoPartParam] = {
    fParteff.partMHistoMin[iPart],     // M
    0.f,                               // p
    0.f,                               // pt
    -3.0f,                             // y
    -10.f,                             // DecayL
  ...
  float xMax[nHistoPartParam] = {
    fParteff.partMHistoMax[iPart],     // M
    10.f,                              // p
    10.f,                              // pt
    3.f,                               // y
    50.f,                              // DecayL
  ...
  for (int iH = 0; iH < nHistoPartParam; iH++) {
    ...
    histoParameters[iPart][iH] = new TH1F(parName[iH].Data(), parTitle[iH].Data(), nBins[iH], xMin[iH], xMax[iH]);
    histoParameters[iPart][iH]->GetXaxis()->SetTitle(parAxisName[iH].Data());
  }

  histoParameters2D[iPart][0] =
    new TH2F(parName2D[0].Data(), parTitle2D[0].Data(), nBins[3], xMin[3], xMax[3], nBins[2], xMin[2], xMax[2]);
  ...
  if (drawZR) {
    histoParameters2D[iPart][1] =
      new TH2F(parName2D[1].Data(), parTitle2D[1].Data(), nBins[12], xMin[12], xMax[12], nBins[13], xMin[13], xMax[13]);
  ...
  if (IsCollectArmenteros(iPart)) {
    histoParameters2D[iPart][2] = new TH2F(parName2D[2].Data(), parTitle2D[2].Data(), 50, -1.f, 1.f, 150, 0.f, 1.f);
  ...
  //create y-mt plot
  histoParameters2D[iPart][3] = new TH2F(parName2D[3].Data(), parTitle2D[3].Data(), nBins[3], xMin[3], xMax[3],  //y
  ...
  if (IsCollectDalitz(iPart)) {
    ...
    histoParameters2D[iPart][4] =
      new TH2F(parName2D[4].Data(), parTitle2D[4].Data(), nBinsM12, m12Min, m12Max, nBinsM23, m23Min, m23Max);
    ...
    histoParameters2D[iPart][5] =
      new TH2F(parName2D[5].Data(), parTitle2D[5].Data(), nBinsM13, m13Min, m13Max, nBinsM23, m23Min, m23Max);
    ...
    histoParameters2D[iPart][6] =
      new TH2F(parName2D[6].Data(), parTitle2D[6].Data(), nBinsM12, m12Min, m12Max, nBinsM13, m13Min, m13Max);
  ...
  if (histoParameters3D && IsCollect3DHistogram(iPart)) {
    histoParameters3D[iPart][0] = new TH3F(parName3D[0].Data(), parTitle3D[0].Data(), nBins[3], xMin[3], xMax[3],
                                           nBins[2], xMin[2], xMax[2], nBins[0], xMin[0], xMax[0]);
    ...
    histoParameters3D[iPart][1] = new TH3F(parName3D[1].Data(), parTitle3D[1].Data(), nBins[3], xMin[3], xMax[3],
                                           nBins[16], xMin[16], xMax[16], nBins[0], xMin[0], xMax[0]);
    ...
    for (int iCH = 0; iCH < 3; iCH++) {
      histoParameters3D[iPart][2 + iCH] =
        new TH3F(parName3D[2 + iCH].Data(), parTitle3D[2 + iCH].Data(), 10, 0., 10., nBins[centralityHisto[iCH]],
                 xMin[centralityHisto[iCH]], xMax[centralityHisto[iCH]], nBins[0], xMin[0], xMax[0]);
    ...
    histoParameters3D[iPart][5] = new TH3F(parName3D[5].Data(), parTitle3D[5].Data(), nBins[5], xMin[5], xMax[5],
                                           nBins[2], xMin[2], xMax[2], nBins[0], xMin[0], xMax[0]);
    ...
    if (IsCollectDalitz(iPart)) {
      ...
      histoParameters3D[iPart][6] = new TH3F(parName3D[6].Data(), parTitle3D[6].Data(), nBinsM12, m12Min, m12Max,
                                             nBinsM23, m23Min, m23Max, nBinsMass, MMin, MMax);
      ...
      histoParameters3D[iPart][7] = new TH3F(parName3D[7].Data(), parTitle3D[7].Data(), nBinsM13, m13Min, m13Max,
                                             nBinsM23, m23Min, m23Max, nBinsMass, MMin, MMax);
      ...
      histoParameters3D[iPart][8] = new TH3F(parName3D[8].Data(), parTitle3D[8].Data(), nBinsM12, m12Min, m12Max,
                                             nBinsM13, m13Min, m13Max, nBinsMass, MMin, MMax);
      ...
      histoParameters3D[iPart][9] =
        new TH3F(parName3D[9].Data(), parTitle3D[9].Data(), nBinsM12, m12Min * m12Min, m12Max * m12Max, nBinsM23,
                 m23Min * m23Min, m23Max * m23Max, nBinsMass, MMin, MMax);
      ...
      histoParameters3D[iPart][10] =
        new TH3F(parName3D[10].Data(), parTitle3D[10].Data(), nBinsM13, m13Min * m13Min, m13Max * m13Max, nBinsM23,
                 m23Min * m23Min, m23Max * m23Max, nBinsMass, MMin, MMax);
      ...
      histoParameters3D[iPart][11] =
        new TH3F(parName3D[11].Data(), parTitle3D[11].Data(), nBinsM12, m12Min * m12Min, m12Max * m12Max, nBinsM13,
                 m13Min * m13Min, m13Max * m13Max, nBinsMass, MMin, MMax);
    ...
    for (int iCH = 0; iCH < 3; iCH++) { histoParameters3D[iPart][2 + iCH] = NULL; }
...
bool KFParticlePerformanceBase::IsCollectZRHistogram(int iParticle) const
{
  ...
  return (abs(fParteff.partPDG[iParticle]) == 310 || abs(fParteff.partPDG[iParticle]) == 3122
          || abs(fParteff.partPDG[iParticle]) == 3312 || abs(fParteff.partPDG[iParticle]) == 3334
...
void KFParticlePerformanceBase::CreateParameterSubfolder(
  ...
  TH1F* histoFit[KFPartEfficiencies::nParticles][nFitQA], int iPart, bool withWrongPVHypothesis)
{
  ...
  gDirectory->cd(folderName.Data());
  {
    ...
    if (histoFit != 0) { CreateFitHistograms(histoFit[iPart], iPart); }
  }
  gDirectory->cd("..");
}
