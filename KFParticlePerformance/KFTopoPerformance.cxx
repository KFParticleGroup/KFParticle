...
KFTopoPerformance::KFTopoPerformance()
  ...
  , MCtoRPVId(0)
  , RtoMCPVId(0)
  , fPrintEffFrequency(1)
  , fCentralityBin(-1)
  , fCentralityWeight(0.f)
  , fEfficiencyMapFile(nullptr)
  , fAcceptanceMapReco(KFPartEfficiencies::nParticles, nullptr)
  , fAcceptanceMapMC(KFPartEfficiencies::nParticles, nullptr)
  , fCutsEfficiencyMapReco(KFPartEfficiencies::nParticles, nullptr)
...
void KFTopoPerformance::Set3DEfficiency(TString fileName = "")
{
  ...
  if (!fEfficiencyMapFile) {
    std::cout << "File defined by the KFTopoPerformance::Set3DEfficiency does not exist. Stop the program."
              << std::endl;
    exit(0);
  }
...
#ifdef KFPWITHTRACKER
void KFTopoPerformance::SetNewEvent(const AliHLTTPCCAGBTracker* const tracker,
                                    ...
                                    AliHLTResizableArray<AliHLTTPCCALocalMCPoint>* localMCPoints)
{
  ...
  for (int iTr = 0; iTr < vMCTracks.size(); iTr++) {
    for (int iP = 0; iP < 3; iP++) { vMCTracks[iTr].SetPar(iP, (*mcTracks)[iTr].Par(iP)); }
    for (int iP = 3; iP < 6; iP++) { vMCTracks[iTr].SetPar(iP, (*mcTracks)[iTr].Par(iP) * (*mcTracks)[iTr].P()); }
  ...
  for (unsigned int iTr = 0; iTr < vMCTracks.size(); iTr++) {
    ...
    if (motherID < 0) {
      ...
      if (newPV) {
        ...
        if (motherID == -1) { primVertex.SetTriggerPV(); }
...
void KFTopoPerformance::GetMCParticles()
{
  ...
  for (unsigned int iMC = 0; iMC < vMCTracks.size(); iMC++) {
    ...
    if (mtra.PDG() == 1010010030) { mtra.SetPDG(3004); }
    if (mtra.PDG() == -1010010030) { mtra.SetPDG(-3004); }
    if (mtra.PDG() == 1010010040) { mtra.SetPDG(3005); }
    if (mtra.PDG() == -1010010040) { mtra.SetPDG(-3005); }
    if (mtra.PDG() == 1003007) { mtra.SetPDG(3007); }
    if (mtra.PDG() == 1003004) { mtra.SetPDG(3012); }
    KFMCParticle part;
  ...
  for (int iMC = 0; iMC < nMCParticles; iMC++) {
    if (abs(vMCParticles[iMC].GetPDG()) == 211 || abs(vMCParticles[iMC].GetPDG()) == 321) {
      ...
      if (muonIndex > -1) {
        int newPDG = 0;
        if (vMCParticles[iMC].GetPDG() > 0) { newPDG = vMCParticles[iMC].GetPDG() + 7000000; }
        else {
    ...
    if (vMCParticles[iMC].NDaughters() >= 2
        && (abs(vMCParticles[iMC].GetPDG()) == 3112 || abs(vMCParticles[iMC].GetPDG()) == 3222
            || abs(vMCParticles[iMC].GetPDG()) == 3312 || abs(vMCParticles[iMC].GetPDG()) == 3334
        ...
        if (abs(vMCParticles[iMC].GetPDG()) == mmMotherPDG[iPDG]) {
          ...
          for (int iMCDaughter = 0; iMCDaughter < vMCParticles[iMC].NDaughters(); iMCDaughter++) {
            ...
            for (unsigned int iPoint = 0; iPoint < xDaughter.size(); iPoint++) {
              ...
              if (isSamePoint) { nDaughtersAtPoint[iPoint].push_back(iMCDaughter); }
          ...
          for (unsigned int iPoint = 0; iPoint < nDaughtersAtPoint.size(); iPoint++) {
            if (nDaughtersAtPoint[iPoint].size() == 2) {
              for (unsigned int iDaughter = 0; iDaughter < nDaughtersAtPoint[iPoint].size(); iDaughter++) {
                ...
                if (abs(vMCParticles[vMCParticles[iMC].GetDaughterIds()[iMCDaughter]].GetPDG())
                    == mmChargedDaughterPDG[iPDG]) {
                  isDaughter[0]     = 1;
                  chargedDaughterId = vMCParticles[iMC].GetDaughterIds()[iMCDaughter];
                }
                if (abs(vMCParticles[vMCParticles[iMC].GetDaughterIds()[iMCDaughter]].GetPDG())
                    == mmNeutralDaughterPDG[iPDG]) {
                  isDaughter[1]     = 1;
  ...
  for (unsigned int iMC = 0; iMC < vMCParticles.size(); iMC++) {
    ...
    //     if(abs(part.GetPDG()) == 4122)
    {
      ...
      for (unsigned int iD = 0; iD < part.GetDaughterIds().size(); iD++) {
        ...
        if (abs(d.GetPDG()) == 3224 || abs(d.GetPDG()) == 3114 || abs(d.GetPDG()) == 113 || abs(d.GetPDG()) == 313
            || abs(d.GetPDG()) == 323 || abs(d.GetPDG()) == 2224 || abs(d.GetPDG()) == 2214 || abs(d.GetPDG()) == 2114
            || abs(d.GetPDG()) == 1114 || abs(d.GetPDG()) == 1000030050 || abs(d.GetPDG()) == 2000020030) {
      ...
      for (int iPDG = indexPDG; iPDG < indexPDG + 10; iPDG++) {
        ...
        if (int(fParteff.partDaughterPdg[iPDG].size()) != nDaughters) { continue; }
        ...
        for (int iDMC = 0; iDMC < nDaughters; iDMC++) { isCorrectPDG &= isDaughterFound[iDMC]; }
...
void KFTopoPerformance::CheckMCParticleIsReconstructable(KFMCParticle& part)
{
  ...
  if (abs(part.GetPDG()) == 211 || abs(part.GetPDG()) == 2212 || abs(part.GetPDG()) == 321 || abs(part.GetPDG()) == 13
      || abs(part.GetPDG()) == 3112 || abs(part.GetPDG()) == 3222 || abs(part.GetPDG()) == 3312
    ...
    if (mcTrack.IsReconstructed()) { part.SetAsReconstructable(3); }
  }
  // tracks
  if (abs(part.GetPDG()) == 211 || abs(part.GetPDG()) == 2212 || abs(part.GetPDG()) == 321 || abs(part.GetPDG()) == 11
      || abs(part.GetPDG()) == 13 || abs(part.GetPDG()) == 1000010020 || abs(part.GetPDG()) == 1000010030
      || abs(part.GetPDG()) == 1000020030 || abs(part.GetPDG()) == 1000020040 || abs(part.GetPDG()) == 1000020060
      || abs(part.GetPDG()) == 1000030060 || abs(part.GetPDG()) == 1000030070 || abs(part.GetPDG()) == 1000040070
    ...
    if (mcTrack.NMCPoints() >= 15) { part.SetAsReconstructable(1); }

    if (mcTrack.IsReconstructed()) { part.SetAsReconstructable(2); }
  }
  //  mother particles
  else {
    ...
    if (part.NDaughters() >= 2) {
      ...
      for (int iD = 0; iD < part.NDaughters(); iD++) {
        ...
        for (int iEff = 0; iEff < 5; iEff++) {
          if (charge > 0) { isPositiveDaughter[iEff] |= daughter.IsReconstructable(iEff); }
          else {
            isNegativeDaughter[iEff] |= daughter.IsReconstructable(iEff);
          }

          if (daughter.IsReconstructable(iEff)) { nRecoDaughters[iEff]++; }
        }
      }
      ...
      for (int iEff = 0; iEff < 3; iEff++) {
        if (nRecoDaughters[iEff] > 1) { part.SetAsReconstructableV0(iEff); }
      }
    }
    ...
    for (int iPart = 0; iPart < fParteff.nParticles; iPart++) {
      if (part.GetPDG() == fParteff.partPDG[iPart]) {
        ...
        for (unsigned int iD = 0; iD < nDaughters; iD++) { pdg[iD] = vMCParticles[part.GetDaughterIds()[iD]].GetPDG(); }

        vector<bool> isDaughterFound(nDaughters);
        for (unsigned int iDMC = 0; iDMC < nDaughters; iDMC++) { isDaughterFound[iDMC] = 0; }
        ...
        for (unsigned int iDMC = 0; iDMC < nDaughters; iDMC++) { isReco = isReco && isDaughterFound[iDMC]; }
...
void KFTopoPerformance::MatchParticles()
{
  ...
  for (unsigned int iRP = 0; iRP < fTopoReconstructor->GetParticles().size(); iRP++) {
    ...
    bool isMissingMass =
      ((abs(rPart.GetPDG()) == 7000211) || (abs(rPart.GetPDG()) == 7000321) || (abs(rPart.GetPDG()) == 7003112)
       || (abs(rPart.GetPDG()) == 7003222) || (abs(rPart.GetPDG()) == 7003312) || (abs(rPart.GetPDG()) == 7003334)
       || (abs(rPart.GetPDG()) == 9000321) || (abs(rPart.GetPDG()) == 8003334) || (abs(rPart.GetPDG()) == 8003222));
    ...
    if ((abs(rPart.GetPDG()) == 7000014) || (abs(rPart.GetPDG()) == 8000014) || (abs(rPart.GetPDG()) == 7002112)
      ...
      if (fNeutralIndex[mcMotherId] > -1) { mcNeutralDaughterId = fNeutralIndex[mcMotherId]; }
      ...
      if (mcNeutralDaughterId > -1) {
        ...
        if (neutralDaughter.GetPDG() == rPart.GetPDG() && neutralDaughter.NDaughters() == rPart.NDaughters()
            && allCorrectDaughters) {
    ...
    //normal decays
      ...
      for (unsigned int ie = 1; ie < mcDaughterIds.size(); ie++) {
        if (mcDaughterIds[ie] == mcDaughterIds[ie - 1]) { nClones++; }
      }
      ...
      if (iD == NRDaughters && mmId > -1) {  // match is found and it is not primary vertex
        ...
        if (mmPart.GetPDG() == rPart.GetPDG() && mmPart.NDaughters() == rPart.NDaughters()) {
...
void KFTopoPerformance::MatchPV()
{
  ...
  for (unsigned int iMCPV = 0; iMCPV < fPrimVertices.size(); iMCPV++) {
    ...
    for (int iD = 0; iD < mcPV.NDaughterTracks(); iD++) {
      if (MCtoRParticleId[mcPV.DaughterTrack(iD)].IsMatched()) { nReconstructedDaughters++; }
    }
  ...
  for (int iPV = 0; iPV < fTopoReconstructor->NPrimaryVertices(); iPV++) {
    ...
    for (int iRP = 0; iRP < nPVTracks; iRP++) {
      ...
      {
        if (motherId == -1) { nTriggerTracks++; }
        if (motherId < -1) { nPileupTracks++; }
    ...
    for (unsigned int iMCPV = 0; iMCPV < nTracksFromMCPV.size(); iMCPV++) {
      ...
      for (unsigned int ie = 1; ie < mcIDs[iMCPV].size(); ie++) {
        if (mcIDs[iMCPV][ie] == mcIDs[iMCPV][ie - 1]) { nClones++; }
      }
...
void KFTopoPerformance::MatchTracks()
{
...
#ifdef KFPWITHTRACKER
  for (int iTr = 0; iTr < vMCTracks.size(); iTr++) {
    if ((AliHLTTPCCAPerformance::Instance().GetSubPerformance("Global Performance")->GetMCData())[iTr]
          .IsReconstructed()) {
  ...
  for (int iTr = 0; iTr < fTrackMatch.size(); iTr++) {
    const AliHLTTPCCAPerformanceRecoTrackData& matchInfo =
      (AliHLTTPCCAPerformance::Instance().GetSubPerformance("Global Performance")->GetRecoData())[iTr];
    if (matchInfo.IsGhost(PParameters::MinTrackPurity) || matchInfo.GetMCTrackId() < 0) { fTrackMatch[iTr] = -1; }
    else {
...
void KFTopoPerformance::CalculateEfficiency()
{
  ...
  for (int iP = 0; iP < NRP; ++iP) {
    ...
    for (int iPart = 0; iPart < fParteff.nParticles; iPart++) {
      if (pdg == fParteff.partPDG[iPart]) { partEff.IncReco(isGhost, isBG, fParteff.partName[iPart].data()); }
    }
  ...
  for (int iP = 0; iP < NMP; ++iP) {
    ...
    if (!(decays.empty())
        && (iParticle[0] < fParteff.fFirstStableParticleIndex || iParticle[0] > fParteff.fLastStableParticleIndex)) {
    ...
    if (fParteff.GetParticleIndex(pdg) >= KFPartEfficiencies::fFirstMissingMassParticleIndex
      ...
      for (int iEff = 0; iEff < 3; iEff++) { isRecPart.push_back(part.IsReconstructable(iEff)); }
    }
    ...
    if (decays.empty() && (part.IsReconstructableV0(0) || part.IsReconstructableV0(1) || part.IsReconstructableV0(2))) {
      ...
      vector<bool> isRecV0;
      for (int iEff = 0; iEff < 3; iEff++) { isRecV0.push_back(part.IsReconstructableV0(iEff)); }
...

    {
      for (unsigned int iPType = 0; iPType < iParticle.size(); iPType++) {
        ...
        partEff.Inc(isReco[iPType], nClones[iPType], isReconstructable[iPType][0], isReconstructable[iPType][1],
                    isReconstructable[iPType][2], fParteff.partName[iPart].data());
        if (mId < 0) {
          partEff.Inc(isReco[iPType], nClones[iPType], isReconstructable[iPType][0], isReconstructable[iPType][1],
                      isReconstructable[iPType][2], (fParteff.partName[iPart] + "_prim").data());
        }
        else {
          partEff.Inc(isReco[iPType], nClones[iPType], isReconstructable[iPType][0], isReconstructable[iPType][1],
                      isReconstructable[iPType][2], (fParteff.partName[iPart] + "_sec").data());
        }
        ...
        for (int iEff = 0; iEff < 3; iEff++) {
          ...
          if (part.NDaughters() > 0
            ...
            if (mcTrack.MotherId() < 0) {
              ...
              for (int iParam = 0; iParam < 6; iParam++) { motherKFParticle.Parameter(iParam) = mcTrack.Par()[iParam]; }
          ...
          if (fStoreMCHistograms) {
            ...
            if (fStore3DEfficiency) {
              ...
              if (iEff == 0) { hPartEfficiencyMulti[iPart][1]->Fill(point, 1); }
...
void KFTopoPerformance::FillParticleParameters(
  ...
  TH1F* histoDSToParticleQA[KFPartEfficiencies::nParticles][nDSToParticleQA], vector<int>* multiplicities)
{
  /** Fills provided histograms with the parameters of the given particle. */
  const std::map<int, bool>& decays = fTopoReconstructor->GetKFParticleFinder()->GetReconstructionList();
  if (!(decays.empty())
      && (iParticle < fParteff.fFirstStableParticleIndex || iParticle > fParteff.fLastStableParticleIndex)) {
    if (decays.find(TempPart.GetPDG()) == decays.end()) { return; }
  }

  if (TempPart.NDaughters() == 1 && TempPart.GetDeviationFromVertex(fTopoReconstructor->GetPrimVertex(0)) > 18.) {
    return;
  }
  ...
  float weight = 1.f;
  if (fAcceptanceMapReco[iParticle] && fAcceptanceMapMC[iParticle] && fCutsEfficiencyMapReco[iParticle]
      && fCutsEfficiencyMapMC[iParticle]) {
    ...
    if (weight > 0.f) { weight = 1.f / weight; }
  }

#ifdef __ROOT__
  if ((l[0] > 0.2f || Pt < 0.f)
      && (abs(TempPart.GetPDG()) == 4122 || abs(TempPart.GetPDG()) == 104122 || abs(TempPart.GetPDG()) == 204122
          || abs(TempPart.GetPDG()) == 304122 || abs(TempPart.GetPDG()) == 404122
          || abs(TempPart.GetPDG()) == 504122)) {
    return;
  }
  if ((l[0] > 0.2f || Pt < 0.f)
      && (abs(TempPart.GetPDG()) == 421 || abs(TempPart.GetPDG()) == 420 || abs(TempPart.GetPDG()) == 425
          || abs(TempPart.GetPDG()) == 426 || abs(TempPart.GetPDG()) == 427 || abs(TempPart.GetPDG()) == 429)) {
    return;
  }
  if ((l[0] > 0.4f || Pt < 0.f)
      && (abs(TempPart.GetPDG()) == 411 || abs(TempPart.GetPDG()) == 100411 || abs(TempPart.GetPDG()) == 200411
          || abs(TempPart.GetPDG()) == 300411)) {
    return;
  }
  if ((l[0] > 0.2f || Pt < 0.f)
      && (abs(TempPart.GetPDG()) == 431 || abs(TempPart.GetPDG()) == 100431 || abs(TempPart.GetPDG()) == 200431
          || abs(TempPart.GetPDG()) == 300431 || abs(TempPart.GetPDG()) == 400431)) {
    return;
  }
  ...
  if (Pt < 0.5f && (abs(TempPart.GetPDG()) == 3000 || abs(TempPart.GetPDG()) == 3001)) { return; }
...
//   if(r < 5 ) return; //TODO collider
  ...
  */
  ...
  if (multiplicities) { multiplicities[0][iParticle]++; }
  ...
  if (histoParameters2D[0][iParticle][1] && drawZR) { histoParameters2D[0][iParticle][1]->Fill(Z, R, 1); }
  ...
  if (TempPart.NDaughters() == 2 && IsCollectArmenteros(iParticle)) {
    ...
    if (index1 >= int(fTopoReconstructor->GetParticles().size())
        || index2 >= int(fTopoReconstructor->GetParticles().size()) || index1 < 0 || index2 < 0) {
      return;
    }
  ...
  if (histoDSToParticleQA && IsCollect3DHistogram(iParticle)) {
    if (fabs(fParteff.GetMass(iParticle) - M) < 3.f * fParteff.GetMassSigma(iParticle))  //SignalReco
    {
      ...
      if (multiplicities) { multiplicities[4][iParticle]++; }

      histoParameters2D[4][iParticle][0]->Fill(Rapidity, Pt, weight);
      histoParameters2D[4][iParticle][3]->Fill(Rapidity, M_t, weight);

      if (drawZR) {
        if (histoParameters2D[4][iParticle][1]) { histoParameters2D[4][iParticle][1]->Fill(Z, R, 1); }
      }
      if (TempPart.NDaughters() == 2 && IsCollectArmenteros(iParticle)) {
        if (histoParameters2D[4][iParticle][2]) { histoParameters2D[4][iParticle][2]->Fill(QtAlpha[1], QtAlpha[0], 1); }
      }
    }
        ...
        && fabs(fParteff.GetMass(iParticle) - M) <= 6.f * fParteff.GetMassSigma(iParticle))  //BGReco
    {
      ...
      if (multiplicities) { multiplicities[5][iParticle]++; }

      histoParameters2D[5][iParticle][0]->Fill(Rapidity, Pt, weight);
      histoParameters2D[5][iParticle][3]->Fill(Rapidity, M_t, weight);

      if (drawZR) {
        if (histoParameters2D[5][iParticle][1]) { histoParameters2D[5][iParticle][1]->Fill(Z, R, 1); }
      }
      if (TempPart.NDaughters() == 2 && IsCollectArmenteros(iParticle)) {
        if (histoParameters2D[5][iParticle][2]) { histoParameters2D[5][iParticle][2]->Fill(QtAlpha[1], QtAlpha[0], 1); }
      }
    }
  }
  ...
  if (!histoDSToParticleQA && iSet == 1) {
    ...
    if (iPV >= 0) {
      if (isSecondaryParticle) { iSet = 4; }
      else {
        int iMCPV = -1;
        if (RtoMCPVId[iPV].IsMatchedWithPdg()) { iMCPV = RtoMCPVId[iPV].GetBestMatch(); }

        int iMCPVFromParticle = fMCTrackToMCPVMatch[iMCTrack];
        if (iMCPV != iMCPVFromParticle) { iSet = 4; }
      }
    }
    else {
      if (!isSecondaryParticle) { iSet = 4; }
  ...
  for (int iParam = 0; iParam < 17; iParam++) { histoParameters[iSet][iParticle][iParam]->Fill(parameters[iParam]); }
  // for(int iParam=0; iParam<8; iParam++)
  //   histoParameters[iSet][iParticle][18+iParam]->Fill(errors[iParam]);

  if (multiplicities) { multiplicities[iSet][iParticle]++; }

  histoParameters2D[iSet][iParticle][0]->Fill(Rapidity, Pt, 1);
  if (drawZR) {
    if (histoParameters2D[iSet][iParticle][1]) { histoParameters2D[iSet][iParticle][1]->Fill(Z, R, 1); }
  }
  if (TempPart.NDaughters() == 2 && IsCollectArmenteros(iParticle)) {
  ...
  if (histoFit) {
    ...
    if (TempPart.NDaughters() == 1) { TempPart.TransportToPoint(decayVtx); }
    ...
    Double_t res[8] = {0}, pull[8] = {0}, mcParam[8] = {mcX, mcY, mcZ, mcPx, mcPy, mcPz, Emc, massMC};
  ...
  if (histoFitDaughtersQA && (TempPart.NDaughters() > 1)) {
    for (int iD = 0; iD < mcPart.NDaughters(); ++iD) {
      ...
      if (iD == 0) { daughterIndex[0] = recDaughterId; }
      if (iD == 1 && daughterIndex[0] > -1 && histoDSToParticleQA) {
        ...
        for (int iDR = 0; iDR < 2; iDR++) {
          for (int iC = 0; iC < 6; iC++) { cD[iDR][iC] += corrCD[iDR][iC]; }
        }
...
void KFTopoPerformance::FillHistos()
{
  ...
  for (int iV = 0; iV < 6; iV++) { multiplicities[iV].resize(KFPartEfficiencies::nParticles, 0); }
  ...
  for (unsigned int iP = 0; iP < fTopoReconstructor->GetParticles().size(); iP++) {
    ...
    FillParticleParameters(TempPart, iParticle, iP, 0, hPartParam, hPartParam2D, hPartParam3D, hFitQA, hFitDaughtersQA,
                           hDSToParticleQA, multiplicities);
  }

  if (fStoreMCHistograms && fStorePrimSecHistograms) {
    for (int iSet = 0; iSet < KFParticleFinder::GetNSecondarySets(); iSet++) {
      const std::vector<KFParticle>& SecondaryCandidates =
        fTopoReconstructor->GetKFParticleFinder()->GetSecondaryCandidates()[iSet];
    ...
    for (int iSet = 0; iSet < KFParticleFinder::GetNPrimarySets(); iSet++) {
      for (int iPV = 0; iPV < fTopoReconstructor->NPrimaryVertices(); iPV++) {
        const std::vector<KFParticle>& PrimaryCandidates =
          fTopoReconstructor->GetKFParticleFinder()->GetPrimaryCandidates()[iSet][iPV];
        for (unsigned int iP = 0; iP < PrimaryCandidates.size(); iP++) {
          ...
          const int id = TempPart.Id();
          FillParticleParameters(TempPart, iParticle, id, iPV, hPartParamPrimaryMass, hPartParam2DPrimaryMass, 0,
                                 hFitQAMassConstraint);

          TempPart = fTopoReconstructor->GetParticles()[id];
          FillParticleParameters(TempPart, iParticle, id, iPV, hPartParamPrimary, hPartParam2DPrimary, 0,
                                 hFitQANoConstraint);
        }

        const std::vector<KFParticle>& PrimaryCandidatesTopo =
          fTopoReconstructor->GetKFParticleFinder()->GetPrimaryTopoCandidates()[iSet][iPV];
        for (unsigned int iP = 0; iP < PrimaryCandidatesTopo.size(); iP++) {
          KFParticle TempPart = PrimaryCandidatesTopo[iP];
          int iParticle       = fParteff.GetParticleIndex(TempPart.GetPDG());
          if (iParticle < 0) { continue; }

          FillParticleParameters(TempPart, iParticle, TempPart.Id(), iPV, hPartParamPrimaryTopo,
                                 hPartParam2DPrimaryTopo, 0, hFitQATopoConstraint);
        }

        const std::vector<KFParticle>& PrimaryCandidatesTopoMass =
          fTopoReconstructor->GetKFParticleFinder()->GetPrimaryTopoMassCandidates()[iSet][iPV];
        for (unsigned int iP = 0; iP < PrimaryCandidatesTopoMass.size(); iP++) {
          KFParticle TempPart = PrimaryCandidatesTopoMass[iP];
          int iParticle       = fParteff.GetParticleIndex(TempPart.GetPDG());
          if (iParticle < 0) { continue; }

          FillParticleParameters(TempPart, iParticle, TempPart.Id(), iPV, hPartParamPrimaryTopoMass,
                                 hPartParam2DPrimaryTopoMass, 0, hFitQATopoMassConstraint);
        }
      }
    }
  }
...
#if 1  //wo MC
    int iParticle = fParteff.GetParticleIndex(fTopoReconstructor->GetParticles()[iP].GetPDG());
    if (iParticle > -1 && iParticle < KFPartEfficiencies::nParticles) { hTrackParameters[iParticle]->Fill(chi2); }
#else
    if (!RtoMCParticleId[iP].IsMatched()) {
  ...
  for (int iPV = 0; iPV < fTopoReconstructor->NPrimaryVertices(); iPV++) {
    ...
    for (unsigned int iZ = 0; iZ < dzPV.size(); iZ++) { hPVParam[14]->Fill(dzPV[iZ]); }
    ...
    if (!RtoMCPVId[iPV].IsMatchedWithPdg()) {
      if (!RtoMCPVId[iPV].IsMatched()) {
        ...
        for (unsigned int iZ = 0; iZ < dzPV.size(); iZ++) { hPVParamGhost[14]->Fill(dzPV[iZ]); }
      }
      else {
        ...
        for (unsigned int iZ = 0; iZ < dzPV.size(); iZ++) { hPVParamBG[14]->Fill(dzPV[iZ]); }
      }
      continue;
    }
    ...
    if (mcPV.IsTriggerPV()) {
      ...
      for (unsigned int iZ = 0; iZ < dzPV.size(); iZ++) { hPVParamSignal[14]->Fill(dzPV[iZ]); }
    }
    else {
      ...
      for (unsigned int iZ = 0; iZ < dzPV.size(); iZ++) { hPVParamPileup[14]->Fill(dzPV[iZ]); }
      iPVType = 1;
    }
    ...
    float dRPVr[3] = {vtx.X() - mcPVx[0], vtx.Y() - mcPVx[1], vtx.Z() - mcPVx[2]};
    float dRPVp[3] = {static_cast<float>(dRPVr[0] / sqrt(errPV[0])), static_cast<float>(dRPVr[1] / sqrt(errPV[1])),
                      static_cast<float>(dRPVr[2] / sqrt(errPV[2]))};

    for (unsigned int iHPV = 0; iHPV < 3; ++iHPV) { hPVFitQa[iPVType][iHPV]->Fill(dRPVr[iHPV]); }
    for (unsigned int iHPV = 3; iHPV < 6; ++iHPV) { hPVFitQa[iPVType][iHPV]->Fill(dRPVp[iHPV - 3]); }
    ...
    hPVFitQa[iPVType][6]->Fill(double(mcPV.NReconstructedDaughterTracks() - fNCorrectPVTracks[iPV])
                               / double(mcPV.NReconstructedDaughterTracks()));
  }
  ...
  if (fStoreMCHistograms) {
    for (unsigned int iP = 0; iP < fTopoReconstructor->GetParticles().size(); iP++) {
      ...
      Double_t res[6] = {0}, pull[6] = {0}, mcParam[6] = {mcX, mcY, mcZ, mcPx, mcPy, mcPz};
  ...
  if (fStoreMCHistograms) {
    for (int iV = 0; iV < 6; iV++) {
      for (int iP = 0; iP < KFPartEfficiencies::nParticles; iP++) {
        if (hPartParam[iV][iP][17]) { hPartParam[iV][iP][17]->Fill(multiplicities[iV][iP]); }
      }
    }
    FillMCHistos();
  }
    ...
    for (int iP = 0; iP < KFPartEfficiencies::nParticles; iP++) {
      if (hPartParam[0][iP][17]) { hPartParam[0][iP][17]->Fill(multiplicities[0][iP]); }
    }
  }
}  // void KFTopoPerformance::FillHistos()
...
void KFTopoPerformance::FillMCHistos()
{
  ...
  for (unsigned int iMCTrack = 0; iMCTrack < vMCTracks.size(); iMCTrack++) {
    ...
    if (part.NDaughters() > 0
        && !(iPDG >= fParteff.fFirstStableParticleIndex && iPDG <= fParteff.fLastStableParticleIndex)) {
      ...
      for (int iP = 0; iP < 6; iP++) { motherKFParticle.Parameter(iP) = vMCTracks[iMCTrack].Par()[iP]; }
    ...
    if (fAcceptanceMapReco[iPDG] && fAcceptanceMapMC[iPDG] && fCutsEfficiencyMapReco[iPDG]
        && fCutsEfficiencyMapMC[iPDG]) {
      const double pointAcceptance[3]{Phi, Theta, P};
...
void KFTopoPerformance::AddV0Histos()
{
  ...
  for (int iV = 0; iV < 4; iV++) {
    for (int iH = 0; iH < nHistoPartParam; iH++) { hPartParam[iV][iV0][iH]->Add(hPartParam[iV][iK0][iH]); }
  }
}
