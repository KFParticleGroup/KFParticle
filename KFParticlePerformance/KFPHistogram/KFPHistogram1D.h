 ...
 ** minimum and maximum value of the axis. The histogram memory is allocated
 *externally
...
public:
  KFPHistogram1D()
      : fHistogram(0), fSize(0), fName(""), fMinBin(0), fMaxBin(0) {}
  KFPHistogram1D(std::string name, int nBins, float minX, float maxX)
      : fHistogram(0), fSize(nBins + 2), fName(name), fMinBin(minX),
        fMaxBin(maxX) {} ///< Constructor with user-defined parameters.
  ~KFPHistogram1D() {}
  ...
  inline void SetBinContent(int iBin, int value) {
    fHistogram[iBin] = value;
  ...
  /** Adds "value" to the histogram: calculates the corresponding bin and adds
   * one there. */
  void Fill(float value) {
    int iBin = floor(float(value - fMinBin) / float(fMaxBin - fMinBin) *
                     float(fSize - 2)) +
               1;
    ...
    if (!(iBin == iBin) || !(std::isfinite(iBin)))
      iBin = 0;
  ...
  inline void operator+=(const KFPHistogram1D &h) {
    if (fSize != h.fSize) {
      std::cout << "Size of 1D histogram " << fName
                << " is incorrect. Stop the program." << std::endl;
    } else {
  ...
  /** The copy-constructor. Memory for the fHistogram is not allocated, only the
   * pointer is copied.*/
  KFPHistogram1D(const KFPHistogram1D &h)
      : fHistogram(h.fHistogram), fSize(h.fSize), fName(h.fName),
        fMinBin(h.fMinBin), fMaxBin(h.fMaxBin) {}

  /** Copies object "h" to the current object. Memory for the fHistogram is not
   * allocated, only the pointer is copied. Returns the current object.  */
...
private:
  ...
  int fSize; ///< Number of bins +2, additional two bins are reserved for the
