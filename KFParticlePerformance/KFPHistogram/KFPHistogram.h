...
public:
  KFPHistogram()
      : fPdgToIndex(), fOutFileName("KFPHistograms.txt"), fMemory(0) {
    KFPartEfficiencies partEff;
    fPdgToIndex = partEff.GetPdgToIndexMap();

    int dataSize = 0;
    for (int iParticle = 0; iParticle < KFPartEfficiencies::nParticles;
         iParticle++) {
  ...
  inline void Fill(const KFParticleTopoReconstructor &topoReconstructor) {
    for (unsigned int iParticle = 0;
         iParticle < topoReconstructor.GetParticles().size(); iParticle++)
      Fill(topoReconstructor.GetParticles()[iParticle]);
  } ///< Fills histograms for each particle reconstructed by the
    ///< KFParticleFinder object from the given KFParticleTopoReconstructor.

  KFPHistogramSet GetHistogramSet(int iSet) const {
    return fKFPHistogramSet[iSet];
  } ///< Returns set of histograms for the decay with index iSet.
  /** \brief Returns "iHistogram" histogram from the set of histograms for the
   * decay with index "iSet". */
  KFPHistogram1D GetHistogram(int iSet, int iHistogram) const {
    return fKFPHistogramSet[iSet].GetHistogram1D(iHistogram);
  }
  ...
  friend std::fstream &operator<<(std::fstream &strm,
    ...
    for (int iParticle = 0; iParticle < KFPartEfficiencies::nParticles;
      ...
      const int &nHistograms =
          histograms.fKFPHistogramSet[iParticle].GetNHisto1D();
      for (int iHistogram = 0; iHistogram < nHistograms; iHistogram++) {
        const KFPHistogram1D &histogram =
            histograms.fKFPHistogramSet[iParticle].GetHistogram1D(iHistogram);
        strm << histogram.Name() << " " << histogram.MinBin() << " "
             << histogram.MaxBin() << " " << histogram.NBins() << std::endl;
  ...
  void Save() {
    ...
    file << (*this);
    file.close();
  } ///< Stores all histograms to the file with the name defined in
    ///< KFPHistogram::fOutFileName.
  ...
  bool FillFromFile(std::string prefix) {
    ...
    for (int iParticle = 0; iParticle < KFPartEfficiencies::nParticles;
      ...
      for (int iHistogram = 0; iHistogram < nHistograms; iHistogram++) {
        ...
        if (nBins != fKFPHistogramSet[iParticle]
                         .GetHistogram1D(iHistogram)
                         .NBins() ||
            minBin != fKFPHistogramSet[iParticle]
                          .GetHistogram1D(iHistogram)
                          .MinBin() ||
            maxBin != fKFPHistogramSet[iParticle]
                          .GetHistogram1D(iHistogram)
                          .MaxBin()) {
          std::cout << "Fatal error: size of the histograms is not in an "
        ...
        for (int iBin = 0; iBin < nBins + 2; iBin++) {
          ifile >> binContent;
          fKFPHistogramSet[iParticle].SetHisto1DBinContent(iHistogram, iBin,
                                                           binContent);
        }
      }
    }
...
private:
  std::map<int, int>
      fPdgToIndex; ///< A map between PDG code and index of the decay in the KF
                   ///< Particle Finder scheme. A copy of an object from
                   ///< KFPartEfficiencies.
  std::string fOutFileName; ///< The name of the output file, where histograms
                            ///< will be stored.
  KFPHistogramSet
      fKFPHistogramSet[KFPartEfficiencies::nParticles]; ///< A set of histograms
                                                        ///< for all decays
                                                        ///< reconstructed by KF
                                                        ///< Particle Finder.
  ...
  KFPHistogram &operator=(const KFPHistogram &); ///< Does not allow copying of
