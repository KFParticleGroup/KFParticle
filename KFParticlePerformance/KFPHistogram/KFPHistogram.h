...
class KFPHistogram {
 public:
  ...
  inline void Fill(const KFParticle& particle)
  {
    ...
    if (it != fPdgToIndex.end()) { fKFPHistogramSet[it->second].Fill(particle); }
  ...
  { return fKFPHistogramSet[iSet]; }  ///< Returns set of histograms for the decay with index iSet.
  ...
  friend std::fstream& operator<<(std::fstream& strm, KFPHistogram& histograms)
  {
    for (int iParticle = 0; iParticle < KFPartEfficiencies::nParticles; iParticle++) {
      ...
      for (int iHistogram = 0; iHistogram < nHistograms; iHistogram++) {
        ...
        strm << histogram.Name() << " " << histogram.MinBin() << " " << histogram.MaxBin() << " " << histogram.NBins()
             << std::endl;
        for (int iBin = 0; iBin < histogram.NBins() + 2; iBin++) { strm << histogram.GetHistogram()[iBin] << " "; }
        strm << std::endl;
      }
    }
  ...
  {
    for (int i = 0; i < KFPartEfficiencies::nParticles; i++) { fKFPHistogramSet[i] += h.fKFPHistogramSet[i]; }
  }  ///< Adds all histograms from object "h" to the current object.

 private:
  std::map<int, int> fPdgToIndex;  ///< A map between PDG code and index of the decay in the KF
                                   ///< Particle Finder scheme. A copy of an object from
                                   ///< KFPartEfficiencies.
  std::string fOutFileName;        ///< The name of the output file, where histograms
                                   ///< will be stored.
  KFPHistogramSet fKFPHistogramSet[KFPartEfficiencies::nParticles];  ///< A set of histograms
                                                                     ///< for all decays
                                                                     ///< reconstructed by KF
                                                                     ///< Particle Finder.
