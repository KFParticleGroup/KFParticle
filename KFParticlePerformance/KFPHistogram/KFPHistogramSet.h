...
class KFPHistogramSet {
 public:
  ...
  {
    int dataSize = 0;
    for (int i = 0; i < NHisto1D; i++) { dataSize += fKFPHistogram1D[i].DataSize(); }
    return dataSize;
  ...
  { return fKFPHistogram1D[iHistogram]; }  ///< Returns one dimensional histogram with index "iHistogram".
  ...
  {
    for (int i = 0; i < NHisto1D; i++) { fKFPHistogram1D[i] += h.fKFPHistogram1D[i]; }
