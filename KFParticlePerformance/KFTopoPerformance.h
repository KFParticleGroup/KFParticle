 ...
 ** For each particle from the KF Particle Reconstruction scheme histograms with
 *parameter distribution,
 ** efficiencies, fit QA, fit QA of daughters, histograms for the side bands
 *method and histograms for
 ** multi-differential extraction of spectra are collected. Also, a set of
 *histograms for quality of
 ** the reconstructed parameters of primary vertices is created: distribution of
 *parameters; fit QA;
 ** fit QA of primary tracks; contamination of ghost, secondary (background)
 *tracks and tracks from
...
private:
   ...
   ** fTopoReconstructor->GetParticles() array for the Monte Carlo particle (or
   *track) with index "i". **/
  std::vector<KFPartMatch> MCtoRParticleId;
  /** Matching between reconstructed and Monte Carlo particles.
   *RtoMCParticleId[i] provides index of the Monte Carlo particle in the
   ** vMCTracks and vMCParticles arrays for the reconstructed particle with
   *index "i" from fTopoReconstructor->GetParticles(). **/
  std::vector<KFPartMatch> RtoMCParticleId;

  /** Matching between Monte Carlo and reconstructed primary vertices.
   *MCtoRPVId[i] provides index of the reconstructed vertex in the
   ** fTopoReconstructor->GetPrimVertex() array for the Monte Carlo vertex with
   *index "i". **/
  std::vector<KFPartMatch> MCtoRPVId;
  /** Matching between reconstructed and Monte Carlo primary vertices.
   *RtoMCPVId[i] provides index of the Monte Carlo vertex in the
   ** fPrimVertices array for the reconstructed vertex with index "i" from
