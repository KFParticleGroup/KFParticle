...
/** @class KFMCVertex
 ** @brief A class to store information about simulated Monte Carlo primary
 *vertices.
 ** @author  M.Zyzak, I.Kisel
 ** @date 05.02.2019
 ** @version 1.0
 **
 ** The class contains coordinates of the vertex, indices of the Monte Carlo
 *tracks produced
 ** at this vertex, classification flags, number of the reconstructed Monte
 *Carlo tracks.
 **/
...
class KFMCVertex {
 public:
  ...
  { return fDaughterTracks.size(); }  ///< Returns number of Monte Carlo tracks produced at the current vertex.
  int NReconstructedDaughterTracks() const
  { return fNReconstructedDaughters; }  ///< Returns number of reconstructed tracks from this vertex.
  void AddDaughterTrack(int iTr)
  { fDaughterTracks.push_back(iTr); }  ///< Adds unique id of the Monte Carlo track produced at the current vertex.
  int DaughterTrack(int iTr) const
  {
    /** Returns unique id of the Monte Carlo track from this vertex with index
     *"iTr".
  ...
  { return fIsMCReconstructable; }  ///< Returns flag showing if the vertex can be found (definition is based on
  ///< the MC tracks)
  bool IsReconstructable() const
  { return fIsReconstructable; }  ///< Returns flag showing if the vertex can be found (definition is based on
  ...
  { fIsReconstructable = 1; }  ///< Defines the current vertex as such that can be reconstructed (based on
  ///< the reconstructed tracks)
  void SetUnReconstructable()
  { fIsReconstructable = 0; }  ///< Defines the current vertex as such that can not be reconstructed (based
  ///< on the reconstructed tracks)

  void SetMCReconstructable()
  { fIsMCReconstructable = 1; }  ///< Defines the current vertex as such that can be reconstructed (based on
  ///< the MC tracks)
  void SetMCUnReconstructable()
  { fIsMCReconstructable = 0; }  ///< Defines the current vertex as such that can not be reconstructed (based
  ///< on the MC tracks)
  ...
  { fIsReconstructed = 0; }  ///< Defines the current vertex as such that was not reconstructed

  void SetNReconstructedDaughters(int n)
  { fNReconstructedDaughters = n; }  ///< Defines number of the reconstructed tracks produced at the current
  ///< vertex.
 ...
 protected:
  ...
  std::vector<int> fDaughterTracks;  ///< Vector with unique ids of the Monte
                                     ///< Carlo tracks produced at this vertex.
  bool fIsReconstructable;           ///< Flag showing if the vertex considered as
                                     ///< reconstructable based on the reconstructed
                                     ///< tracks.
  bool fIsMCReconstructable;         ///< Flag showing if the vertex considered as
                                     ///< reconstructable based on the Monte Carlo
                                     ///< tracks.
  bool fIsReconstructed;             ///< Flag showing if vertex was reconstructed.
  int fNReconstructedDaughters;      ///< Number of found tracks, produced at the
                                     ///< current vertex.
  bool fIsTriggerPV;                 ///< Flag showing if the vertex was a trigger primary
                                     ///< vertex.
};
