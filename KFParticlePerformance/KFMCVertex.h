...
public:
  ...
  int DaughterTrack(int iTr) const {
    ...
    if (iTr >= NDaughterTracks()) {
      std::cout << "ERROR!!!! MC PV contains only " << NDaughterTracks()
                << " tracks" << std::endl;
      return -1;
    }
  ...
  bool IsMCReconstructable() const {
    return fIsMCReconstructable;
  } ///< Returns flag showing if the vertex can be found (definition is based on
    ///< the MC tracks)
  bool IsReconstructable() const {
    return fIsReconstructable;
  } ///< Returns flag showing if the vertex can be found (definition is based on
  ...
  } ///< Defines the current vertex as such that can be reconstructed (based on
    ///< the reconstructed tracks)
  void SetUnReconstructable() {
    fIsReconstructable = 0;
  } ///< Defines the current vertex as such that can not be reconstructed (based
    ///< on the reconstructed tracks)

  void SetMCReconstructable() {
    fIsMCReconstructable = 1;
  } ///< Defines the current vertex as such that can be reconstructed (based on
    ///< the MC tracks)
  void SetMCUnReconstructable() {
    fIsMCReconstructable = 0;
  } ///< Defines the current vertex as such that can not be reconstructed (based
    ///< on the MC tracks)
  ...
  void SetNReconstructedDaughters(int n) {
    fNReconstructedDaughters = n;
  } ///< Defines number of the reconstructed tracks produced at the current
    ///< vertex.
