  ...
  std::ostream& operator<<(std::ostream& os, const KFParticle& particle)
{
  ...
  for (Int_t i = 0; i < 8; i++) {
    ...
    if (particle.GetCovariance(i, i) > 0) {
      os << " " << vn[i] << ": " << std::setw(8) << particle.GetParameter(i) << " +/- " << std::setw(6)
         << sqrt(particle.GetCovariance(i, i));
    }
    else {
  ...
  os << " pdg:" << std::setw(5) << particle.GetPDG() << " Q: " << std::setw(2) << int(particle.GetQ())
     << " chi2/NDF: " << std::setw(8) << particle.GetChi2() << "/" << std::setw(2) << particle.GetNDF();
  return os;
}
...
void KFParticleTest::RunTestSingle()
{
  ...
  vert.SetCovarianceMatrix(0.01, 0.00, 0.01, 0.00, 0.00, 0.01);
  ...
  KFPTrack track;
  track.SetParameters(-0.061996019110347252, -1.3579236865955473, 27.147283554077148, 0.62539337626870062,
                      -0.028552340672283318, -0.18467358509984011);
  float C[21] = {3.3055800809774214e-05, 0.00098316976438185002,  0.04740889543423539,     -8.5596097466772512e-05,
                 -0.0037516094381694971, 0.032156504690647125,    -2.2812597903705375e-05, -0.0012121012247057524,
                 3.0646383360925928e-05, 6.1388628418184652e-05,  -4.4071909055788304e-06, -0.00048870318030618627,
                 3.8062554692505919e-05, 1.2177141510445709e-05,  7.6900178535210476e-06,  6.6224441962932268e-06,
                 0.00034363110217286891, -0.00031520420397528146, -1.6277704753223909e-05, -3.4322154557097545e-06,
                 1.027411488502718e-05};
  ...
  std::cout << std::endl
            << "3. Now we will create one more particle from track and call the construction from these 2 particles"
            << std::endl
            << std::endl;

  KFPTrack track2;
  track2.SetParameters(-0.20371287092090862, 3.0678058943547839, -19.93988037109375, 0.37533048135363339,
                       0.024923235867488316, 0.19031024520542122);
  float C2[21] = {0.00022312908970259721,  -0.00064291160449645151, 0.089331037457232143,    0.00047880877483649206,
                  -0.045478494677353445,   0.11199165135622025,     4.6362085390124077e-07,  0.00070978326424729935,
                  -0.00014164977426380486, 1.7553871209443515e-05,  -2.2044831998838091e-05, -0.00059994741249631909,
                  0.00030148707952079015,  -4.6574515272730461e-06, 7.2618497455845866e-06,  -1.2427988441207971e-06,
                  0.00030830063771211896,  -0.00061853865528922161, 5.4390968700069889e-06,  -1.9914477627292868e-06,
                  8.9837108094398403e-06};
  ...
  std::cout << "Dauthers" << std::endl << "  " << *vDaughters[0] << std::endl << "  " << *vDaughters[1] << std::endl;
